...
注記(2026-07-26): Codexは開発工程から外れ、`recovery_exec`とProxmoxパッチ適用の実行エンジンとしてのみ稼働している。それらは`recovery_exec_setup`が配布する専用のAGENTS.md(`AGENTS.md.j2`)を読むため、このリポジトリ直下のAGENTS.mdを参照する主体は現状存在しない。本ファイルの要否は未判断。
