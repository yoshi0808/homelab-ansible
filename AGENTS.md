...
2. **自分のRole本文** — **起動時に指定された役**の `docs/ai/roles/<role>.md` を読む。**Codexが担うのはImplementer・計画Reviewer・Auditorであり、Coordinatorではない**(割り当ての正本は [`docs/ai/roles/coordinator.md`](docs/ai/roles/coordinator.md))。**役の指定が無いときはCoordinatorとして振る舞わず、どの役かを確認してから着手する。**
