...
from ansible.module_utils.quory_health_files import locked, read_json, save_json, report_path, plain
from ansible.module_utils.quory_health import markdown
...
def publish(root, report, parent, client):
    if not parent:
        raise PublishError("parent_unconfirmed")
    month = report["month"]
    title = "quoryヘルス月次点検 " + month
    marker = "quory-health-monthly:v1:" + month
...
def main():
    ...
    try:
        with locked(module.params["directory"]) as root:
            ...
            client = Client(module.params["token_file"]) if module.params["parent"] else None
            if client is None:
                raise PublishError("parent_unconfirmed")
