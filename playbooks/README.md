...
| [`proxmox_storage_monthly.yml`](proxmox_storage_monthly.yml) | `localhost`, `storage_monthly_targets`, `localhost` | inventoryのproxmoxを対象に月次ZFS/NVMe点検、履歴比較、Notion正式レポート | `check-mode-native` | `proxmox_storage_monthly` |
...
| [`semaphore_db_backup.yml`](semaphore_db_backup.yml) | `pve1`/`pve2`（選定）、quory(delegate_to) | quoryのSemaphore project backup API JSON（template/scheduleを検証）と参考用config.jsonを2点1世代でSynology NFSへ退避、原子的確定・世代ローテーション | `risk-accepted` | `semaphore_db_backup` |
