...
`monitoring_healthcheck.yml`（monnie向けサービス状態収集）を新規実装した際、当初 `prometheus / grafana-server / loki` の3サービスのみを対象としており、`unpoller.service` が含まれていなかった。

**問題:** `_is_unhealthy("monnie_healthcheck", result)` は `data.services` の全値をチェックするが、unpollerが収集対象になければ停止しても「全サービス active」と判定され、復旧がトリガーされない。

**発見経緯:** 実機テスト時に unpoller を停止してSlackから復旧依頼を送ったところ、正常との回答が返ってきた。調査の結果、直前の手動パイプラインテストで unpoller が再起動されており、その後の Slack テストでは既に復帰済みだったことが判明。スクリプト内容を確認し、`unpoller.service` が収集されていることを確認した（既に追加済みだった）。

**教訓:** 新規 healthcheck を作成する際は、§2.1 の systemdユニット対応表と照合し、対象ユニットが全て収集されていることを確認する。healthcheck の収集対象と `_is_unhealthy()` の判定対象は一致させる。
...
### 14.4 ansible-playbook の become_user に ACL 問題
