...
|---|---|---|---|---|
| 調査用キー | `recovery-exec` | authy / monnie（共用、1本。sophos-fw / pve1 / pve2 は対応予定） | 1 | パラメータ受領可。ノード側wrapperが許可リスト照合（§4.4） |
| recovery_action用キー | `recovery-exec` | authy / monnie（共用、1本。annとは別系統。pve1 / pve2 は対応予定） | 1 | パラメータ不可。完全固定文字列のみ（§4.3） |
...
```text
command="<recovery-action script>"（authy向け: freeradius を1本で固定）
command="<recovery-action script>"（monnie向け: grafana-server/prometheus/loki/unpoller を
  一括で1本固定。個別指定なし）
command="qm reboot <固定vmid>"（VMリブート用、対象ごとに固定）（対応予定）
command="ha-manager crm-command relocate vm:<固定vmid> <固定node>"
  （フェイルオーバー用、対象ごとに固定）（対応予定）
```
...
1回のSlack依頼・1回のトリガー内での復旧試行は**1回のみ**とする。
フロー: 調査 → 異常確認 → homelab-recover-* 実行（1回） → 再調査 → それでも異常 → 即エスカレーション。
再restart（2回目の homelab-recover-* 実行）は行わない（§10「各段1回のみ試行」、§13禁則と一致）。

```text
wrapperレベル（本セクション）: 1回のSlack依頼・1回のトリガー内での
  調査→復旧→再調査サイクル（復旧試行は1回のみ）
§9のflapping対策: 複数回のトリガーに渡る、24時間単位の発生回数制限
```

両者は矛盾せず、内側（1回の対応の中での即エスカレーション）と外側（繰り返し発生する障害への対応）として独立に機能する。
...
```text
sandbox: workspace-write（workspace 内のみ書き込み可。workspace 外への書き込みは禁止）
...
|---|---|
| §8.3 Slackメンション | 実装済み |
| §8.1 OnFailure= 自動通知 | 将来対応 |
| §8.2 probe 失敗検知 | 将来対応 |
| §8.4 前提チェック（mute / flapping / lock） | 将来対応 |

### 8.1 authy / monnie → quory（systemd OnFailure=）【将来対応】
...
### 8.2 外部到達性低下時（probe失敗）【将来対応】

quory上でping probeを実行し、失敗時に検知パイプラインを直接呼ぶ。

### 8.3 Slackメンション経由（手動・エージェント的ループ）【実装済み】
...
**現在の実装範囲（Slack経由、§8.3）**: サービスrestart（homelab-recover-* wrapper 経由でCodexが判断・実行）。

**将来対応**: VMリブート・HAフェイルオーバー（wrapper未実装）。§8.1 の自動トリガー経路実装時に合わせて対応する。
...
| 仕組み | 目的 | 性質 | 実装状況 |
|---|---|---|---|
| 実行中ロック | 並行実行防止 | アトミック（mkdir方式） | 将来対応 |
| 直近24時間のトリガー回数 | 繰り返し失敗時の暴走防止 | 回数ベース、3回以上で即エスカレーション | 将来対応 |

**現在**: recovery_io.py に実行中ロック・flapping count は未実装。§8.1 自動トリガー経路の実装時に合わせて対応する。§8.3（Slackメンション）経由では人間の明示依頼のためflapping countをスキップする仕様（§8.3参照）。
...
- quory 側の id_ann・vault pass・専用鍵（§4.2, §4.3）について、ansy で実施したのと同じ
  OS権限検証（低権限ユーザーからPermission deniedになることの確認）を再実施する
