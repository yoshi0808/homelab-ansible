...
- 同一 run_loop 内で同一アクションを2回以上実行することを許可する
  （Python 側で history を走査し、既に実行済みなら機械的に拒否する実装を維持すること。
    プロンプト指示のみに依存しない。trigger ガードと同じ方針）
...
- **`ANTHROPIC_API_KEY` が環境変数に存在する場合、OAuth トークンより API キーが優先される**（公式仕様）。
  他ツールの設定ミスや env 継承の誤りで `ANTHROPIC_API_KEY` が紛れ込むと、静かに API 従量課金に切り替わる。
  `recovery-slack` サービスの環境変数（`/etc/recovery/keys/slack-listener.env` および systemd unit）に
  `ANTHROPIC_API_KEY` が存在しないことを定期的に確認すること。
