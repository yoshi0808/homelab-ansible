...
VM / CT home node 定義は、以下のファイルに分離する。

```text
inventories/homelab/vars/proxmox_vm_home_nodes.yml
```

このファイルを VM / CT 配置情報の master とする。  
同じ意味の情報を複数箇所に重複して持たない。

VM 退避・復帰系 playbook は、このファイルを明示的に `include_vars` して使う。

例:

```yaml
- name: Load Proxmox VM/CT home node definitions
  ansible.builtin.include_vars:
    file: "{{ inventory_dir }}/vars/proxmox_vm_home_nodes.yml"
```
...
proxmox_vm_home_nodes:
  "101":
    display_name: "example-vm-101"
    type: "vm"
    home_node: "pve1"
    node_comment: "pve1.internal / primary node"
    note: "通常時はpve1へ戻す"

  "102":
    display_name: "example-vm-102"
    type: "vm"
    home_node: "pve1"
    node_comment: "pve1.internal / primary node"
    note: "通常時はpve1へ戻す"
  ...
  "201":
    display_name: "example-ct-201"
    type: "ct"
    home_node: "pve2"
    node_comment: "pve2.internal / secondary node"
    note: "通常時はpve2へ戻す"
```

Ansible の判定、VM / CT の復帰処理、report / mail の人間向け表示は、すべてこの定義を参照する。

Ansible が使う値:

```text
proxmox_vm_home_nodes[vmid].home_node
```
...
VM 101 example-vm-101:
  current node: pve2
  home node:    pve1  # pve1.internal / primary node
...
3. pve1 / pve2 healthcheck が OK であることを確認する
4. 対象ノードの最新 dry-run Status が `PATCH_READY` であることを確認する
5. 対象ノード上に running VM / CT が残っていないことを確認する
6. 対象ノードに `apt-get dist-upgrade` を実行する
7. apply 結果を保存する
8. reboot-required があれば対象ノードを reboot する
9. SSH / Proxmox API / GUI の復帰を待つ
10. 対象ノードの post-healthcheck を実行する
11. 結果を report に保存する
12. summary mail を生成する
...
3. pve1 healthcheck
4. pve2 healthcheck
5. どちらか WARNING / CRITICAL なら停止
6. target node patch dry-run
7. Status 判定
8. PATCH_READY 以外なら apply しない
9. target node 上の VM / CT 所在確認
10. target node 上の VM / CT を反対側ノードへ退避
...
1. control node の所在確認
2. target node が control node の稼働ノードではないことを確認
3. pve1 / pve2 healthcheck OK
4. target node dry-run status PATCH_READY
5. target node VM / CT 所在確認
6. target node VM / CT 退避
7. target node apply
8. target node reboot if required
9. target node post-healthcheck OK
10. summary mail
```
...
2. `inventories/homelab/vars/proxmox_vm_home_nodes.yml` を作成
