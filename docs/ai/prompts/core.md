...
- `techlead`（macOS ネイティブアプリ経由の別セッション、同一ホスト）: **壁打ち相手**。案件を
  直接依頼されることもある。画面操作性・画像貼付・遠隔が強み。agmsg 名を `claude` と分けて
  宛先衝突を回避する。

両セッションの運用ルール:

- **memory / リポは共有実体**。ただし memory はセッション開始時に読み込むため、走行中の相手の
  書き込みは、受け手が該当ファイルを**再読して同期**する。セッション間の連絡は agmsg（直通）で
  行い、Yoshinobu を伝令役にしない。書く前に相手の編集有無を確認し、上書き事故を避ける。
- **memory の清書・統合は `claude` が担当**。`techlead` は壁打ちで得た知見を agmsg で `claude`
  に渡し、清書は `claude` が行う（短い自メモの直書きは可）。
- **Codex（implementer / reviewer / tester）へのキックと、確認プロンプトの Y/Enter 代行は、
  そのタスクを依頼した側が所有する**。同一ペインを二人で承認しない。引き継ぎ時は所有権も受け手へ
  移り、元の側は手を引く。代行の可否は「製品名」でなく「**人間 GO 未取得の破壊的な実ホスト変更か**」
  で判断する（GO 済みタスク内のコマンド承認は代行可、未 GO の破壊的操作は不可）。
- Yoshinobu は `claude` / `techlead` へ**同一案件を同時に依頼しない**。外出時などの
  `claude`→`techlead` 逐次引き継ぎはあり得るが、同時依頼にはならない。
- commit / push はどちらのセッションも実行しない（§17）。最終的な commit は Yoshinobu。
