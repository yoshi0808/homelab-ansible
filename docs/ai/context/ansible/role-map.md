...
| `incident_capture` | quory常駐の障害証拠バンドル収集器(evidence collector)を配備(ADR-003) | `roles/incident_capture/defaults/main.yml` のsnapshot対象/schedule/パス、`files/incident-capture-collector.py`、T1(`roles/common_slack/tasks/capture.yml`)が書くspoolレコード、既存の`homelab-semaphore-query`/`homelab-investigate-{host}` | `reports/incidents/`・`_spool/` ディレクトリ作成(create-only)とrecovery-exec宛ACL付与、collector script/config/systemd unit配備、`incident_capture_service_enabled=true`時のみtimer enable+start | `playbooks/incident_capture_setup.yml` |
| `incident_sync` | quory→ansy証拠バンドルの定期ミラー同期(pull-only、read-only at source)と同期状態記録・通知・fail | `roles/incident_sync/defaults/main.yml` の転送元/着地パス・rsync除外・heartbeat閾値・timeout、play1(quory)が残すtransfer facts、ミラーの`_heartbeat.json` | ansy側ミラーディレクトリへの`ansible.posix.synchronize`(rsync pull)、`_sync/last-sync.json`書き出し、スコープ限定git status検査、失敗/stale時Slack通知、失敗時fail。`install_timer`ではservice/timer unit配備・enable/start | `playbooks/incident_sync.yml`, `playbooks/incident_sync_timer.yml` |
| `knowledge_review` | 月次Knowledge振り返り(仕分け・昇格判断)を`claude -p`で無人実行し、続けて障害の自動評価(incident_metrics→incident_evaluation→incident_index_write)を行う | `roles/knowledge_review/defaults/main.yml` のrepo/state dir・claude bin/model/timeout・schedule、auto-memory、ミラー(`reports/incidents/quory/`) | 作業ツリーcleanチェックによる中止判定、auto-memoryスナップショット、allowlist限定の`claude -p`実行、MEMORY.md次回日付更新、report保存、`_evaluations/`配下index.json書き出し、Slack通知。`install_timer`ではservice/timer unit配備 | `playbooks/knowledge_review.yml`, `playbooks/knowledge_review_timer.yml`, `playbooks/incident_evaluation.yml` |
