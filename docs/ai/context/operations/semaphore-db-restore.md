...
- 対象ホストに既に別の Semaphore project が存在する場合(id は固定値として扱わない。`docs/ai/context/system/semaphore.md`)、差し替えは**そのインスタンス自体を quory の状態で上書きする**ことを意味する。検証用途で使い捨てるのでない限り、この経路を本番の別ホストへ直接使わない。
