...
- IPアドレスをinventory、変数、コード、レビュー文書を含むリポジトリ内へ直接記載しない。DNS名または実行時の名前解決を使う。例外はループバック・ワイルドカード・ブロードキャストの3リテラルだけで、これらは環境を指さない(pre-commitもこの3つだけを通す)。
...
quory側には、Yoshinobuの本番運用上の判断と作業を支援するOperatorを置く。Operatorは本番環境の状態を踏まえて調査、実行可否の整理、結果報告、開発側への修正依頼を支援する。具体的な責務、権限、起動方法および安全制約はOperatorのRole文書と個別Policyで定める。
...
以下はAIの振る舞いではなく、**書いたコード・playbook・文書そのものが持つべき性質**である。個別Policyはこの原則の、その領域での適用を書く(原則そのものを言い直さない)。

**判定できないときは止める。** 前提が満たせない、対象が見つからない、状態をどちらとも解釈できない — これらを正常扱いにしない。**空の成功、無言の完走、既定値へのフォールバックで先へ進めない。** 壊れているときほど「問題なし」に見える設計を作らない。判定不能は明示的な失敗として観測できる形にする。

**値を二重に持たない。** ある値・一覧・分類の正本を1つ決め、他の場所はそこを指す。**複製した時点で、片方だけが直る経路ができる。** 正本が別にあるものを説明の都合で書き写さない。書き写す必要を感じたら、それは正本の置き場が間違っている信号として扱う。
