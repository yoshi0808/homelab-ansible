...
| C17-01 | §17: Claude Codeはssh、commit/push、無確認playbook、実host ad-hocを行わない | 製品固有禁止 | 移す | product差分 / Policy | **[既存]** commit/push禁止は`docs/ai/core.md`、**実host ad-hoc禁止は2026-07-30に撤廃**(Coordinatorの確認行為を塞ぐと推測が入るため。代わりに`docs/ai/roles/coordinator.md`が「推測は推測と申し添える」を要求し、受入条件の検証をTesterへ残すことは`skills/delegation-tier/SKILL.md`が持つ。保護対象ホストへの非冪等操作は`.claude/settings.json`の`autoMode.soft_deny`が機械的に止める)、無確認playbook禁止は`docs/ai/policies/ansible_test_safety_policy.md` TS-021。**ssh直接実行禁止は2026-07-26に廃止**(承認境界をCoordinatorへ移管、`~/.claude/settings.json`のssh askも削除) | Codex共通原則へ混ぜない |
