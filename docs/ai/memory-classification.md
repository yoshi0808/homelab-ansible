...
**本節が契約の正本である。** 各lessonの`## 再発記録`節はCoordinatorだけが書く。節の中身は見出し、正本へのポインタ、表だけとし、この契約を各ファイルへ複製しない。

Coordinatorは**各対話セッションを終える前、およびsubagentの成果物を統合するとき**、自分の実行結果と成果物の根拠を現物で確認し、**次のいずれかが実際に起きたときだけ**1行足す。subagentの報告は証拠の入力であって自動採用しない。該当が無ければ何もしない。

1. Policy(`docs/ai/policies/*_policy.md`)の許可・禁止・停止条件に反した。
2. 実行中のCLIまたはharnessの安全機構(permission / approval classifier、deny rule等)に止められた。
3. 規範文書または依頼文に書いてあることをしなかった。

**話題がlessonに似ていることは記録の理由にならない。** 調べた・検証した・見つけた、は記録しない。lessonを正しく適用できているものも記録しない。**反した規範の所在を書けない項目は記録しない。**

**再発記録の行数は、Coordinatorが確認して記録できた事例数であって、実際の発生総数ではない。** 見落としや未観測があり得るため、**回数だけを昇格の根拠にしない。**

いずれのCLIのSessionEnd hookにも、subagentにも、この判定と書込みを担わせない。
