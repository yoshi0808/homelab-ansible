...
当初この振り返りは`incidents/`だけを対象としていたが、**本番影響が出ずIncidentにならなかった学びが網から漏れる**ことが判明したため対象を広げる。2026-07-26にフル工程で回した案件では、工程内で実バグ4件を検出・修正して本番影響ゼロだったためIncidentを1件も作っておらず、そこで得た教訓はCoordinatorの個人memory(Claude Memory、subagentからは読めない)にしか存在しなかった。「苦労した」という体験が学習として定着しないまま流れる状態である。
