...
- **規範文書の変更をレビューするときは`skills/document-norm-review/SKILL.md`を併用する。** Policy / Context / Role文書 / SKILL.md / prompt / AGENTS.mdの移設・削除・正本の差し替え・判定基準の改訂が対象で、コード差分とは欠陥クラスが異なる(宙ぶらりん参照、規範の消失、撤回した根拠の残存)。
