...
| # | 基準 | 差し戻す条件 |
|---|---|---|
| 1 | 未決定の数 | 1単位に未決定の設計判断が2つ以上あれば差し戻す |

**2026-07-29、単位の大きさに関する数値基準(旧: 実行単位80 `tool_uses`超で分割、理想30〜40)を廃止した**(Yoshinobu明示: 「上限などの設定もCoordinatorが決めるべきで、ルールは不要」)。根拠は`docs/ai/status.md`の記帳から見えた実測——単位ごとの比が0.43〜4.44(10.3倍)まで開き、見直しトリガ(1.8倍)を超えていた——であり、`tool_uses`は単位分割の精密なゲートには不向きと判断した。**単位の大きさはCoordinatorが計画時に判断する**(1単位=1つの独立した関心事、独立してレビュー・検証・並列化できる粒度、が目安)。
