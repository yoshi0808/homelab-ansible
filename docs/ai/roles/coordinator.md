...
**着手前の計画はTierの規模で省かない**（2026-07-29 Yoshinobu表明。旧版は「着手前」をsubagent起動単位で書いていたため、subagentを立てないTier 1/2では該当なしと読め、実際に計画が一切説明されないまま実装・commitまで進んだ）。Tier 1/2の計画は承認待ちにはしない — **出したうえでそのまま進めてよい**が、出さずに進めない。これは `skills/delegation-tier/SKILL.md`「判定はいつ行うか — 着手前に必ず声に出す」が要求している宣言の置き場でもある。**最終報告に後から書くのは、判定の記録であって計画ではない。**
