...
**Why**: 2026-07-25、`context/operations/ubuntu-vm-patch.md`のScheduleテーブルがsystemd timer 3件を稼働中(毎日03:30 / 05:30 / 05:35)と記載していたが、実体は`roles/systemd_timers/defaults/main.yml`でコメントアウト済み・Semaphore schedule移行済みだった。同日のPolicy見直しで、Contextが積極的に誤誘導する状態を修理する作業が発生した。2節は「変化の速い値」を書かないと定めていたが、本3節が秘密情報4種類しか列挙していなかったため、時刻を書く余地が残っていた。
