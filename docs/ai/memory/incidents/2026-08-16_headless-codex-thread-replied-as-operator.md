...
1. Coordinator から `homelab-ops` へ1通送り、**返信が来ないこと**
2. `run/codex-bridge.homelab-ops.operator.log` に**新たな `resumed thread` が出ないこと**

## 残っている調査

**boot 後に `operator-runtime` で codex セッションを起動しているものが特定できていない。** サーバ側アクセスログでは、再起動による無音は **42秒**(21:07:25 → 21:08:07 JST)で、engine は boot のおよそ18秒後に戻っていた。人がセッションを立てるには早すぎる。

**起動元が残る限り、seat が再び記録されれば同じことが起きうる。** 削除は再発を防ぐのではなく、いま動いているものを止めただけである。
