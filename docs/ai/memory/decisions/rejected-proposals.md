...
- **agmsg v1.3.1の`fix`不具合を上流(`github.com/fujibee/agmsg`)へissueとして出すこと** — 2026-09-19 Yoshinobu不採用。**外部issueを出した前回の経験を踏まえた判断であり、不具合の当否ではない。** 影響はペインを名指しする操作(`peek` / `poke` / `arrange` / `despawn`)に限られメッセージ経路に及ばないことを実測済みで、**ローカルパッチも当てない**(次のアップデートで消え、消えたことに気づく経路が無い)。**制約を認識したうえで運用を継続する**(Yoshinobu)。原因の連鎖は`docs/ai/status.md`のagmsg段落が持つ
- **Codex向けAGENTS.mdをドリフト検査の対象にすること** — 2026-08-25 見送り(Yoshinobu)。**食い違ったことが一度も観測されていない。** 案件を起こして requirement まで書いたが(`docs/ai/reviews/agents_md_drift_coverage/`)、実装は入れずに畳んだ。**取れない理由が2つある。** ①**AGENTS.mdは2つあり**(`recovery_exec` 側と `incident_inspect` 側)、**後者は変数を持つので静的化できない** — 片方だけ Tier 1 へ入れると「AGENTS.mdは検査されている」と誤読される ②配備タスクが `when: not ansible_check_mode` で守られているため、**`template` を check mode で走らせて `changed` を見るという最も素直な機構が塞がれている**(規約が、検知したい当のタスクを黙らせる)。**dispatchとの同期は `AR-028` が既に規定しており、2026-08-25に測って健全だった**(class G 12本・class P 27本とも記載あり)。**再開の条件**: 実際に1回でも食い違いが観測されたとき。**2つあることと同一化しないことは `AR-104` が持つ**
- **再発記録を「誤りの型」ごとに集約する受け皿を `lessons/` へ新設すること** — 2026-08-25 見送り。11行中9行が「確認せず推測で断定した」型で、かつ**機構がその集中を数えられない**のは事実である(分類器は「同じ lesson は1回だけ」で割り当てるため、同型が複数あると必ず別ファイルへ散る。閾値は file 単位なので原理的に発火しない)。**それでも作らない理由は2つ。** ①9行は原因が2クラス混在しており(観測できずに推測した / 確認手段があったのに確認しなかった)、**前者は `grant-read-access-rather-than-police-speculation` で消えるため母集団がこれから変わる。変わる前にカテゴリをファイル構造へ固定すると、原因を混ぜたまま数える形を作り込むことになる。** ②集めた先の出口(「3回を超えたら Skill 化の候補として人へ出す」)は、この型に対しては行き止まりである(自分で破れる対策は対策でない)。**再開の条件**: アクセス追加後も「確認手段があったのに確認しなかった」が残り、かつ散っているために月次で見えないと分かったとき。**2026-09-07追補:** 自動分類器は廃止され、Coordinatorが確認済み事例を既存lessonへ記録する形になったため、「分類器が別fileへ散らす」という再開条件の前提は終了した。新しい集約用受け皿は作らない判断を維持する
