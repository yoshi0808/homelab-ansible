...
- **再発記録を「誤りの型」ごとに集約する受け皿を `lessons/` へ新設すること** — 2026-08-25 見送り。11行中9行が「確認せず推測で断定した」型で、かつ**機構がその集中を数えられない**のは事実である(分類器は「同じ lesson は1回だけ」で割り当てるため、同型が複数あると必ず別ファイルへ散る。閾値は file 単位なので原理的に発火しない)。**それでも作らない理由は2つ。** ①9行は原因が2クラス混在しており(観測できずに推測した / 確認手段があったのに確認しなかった)、**前者は `grant-read-access-rather-than-police-speculation` で消えるため母集団がこれから変わる。変わる前にカテゴリをファイル構造へ固定すると、原因を混ぜたまま数える形を作り込むことになる。** ②集めた先の出口(「3回を超えたら Skill 化の候補として人へ出す」)は、この型に対しては行き止まりである(自分で破れる対策は対策でない)。**再開の条件**: アクセス追加後も「確認手段があったのに確認しなかった」が残り、かつ散っているために月次で見えないと分かったとき
