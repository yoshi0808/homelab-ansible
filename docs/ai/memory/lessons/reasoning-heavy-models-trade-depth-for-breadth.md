...
- **判断・計画・技術的前提の反証はCoordinator(上位モデル)に残す。** 今回の教訓は「上位モデルが劣る」ではなく「上位モデルにも不得意な作業類型がある」という話である。(この配分を決めたのは当時のTier制度下だが、**Tier制度は2026-07-31に廃止され、工程の組み立てはCoordinatorの裁量になった**。配分の考え方だけが残っている。当時の議論は `docs/ai/reviews/process_retrospective/2026-07-29_008_coordinator_model_tier_policy.md`。)
...
データ点はこのセッション1つ(2026-07-29)である。仮説として扱い、他のセッション・他の案件でも同型の失敗が観測されるかを見ていく。**現行のRoleごとのmodel/effort配分は `docs/ai/roles/coordinator.md`「モデル・effort配分」が正本**であり、選定理由は `docs/ai/adr/010-role-model-effort-allocation.md` が持つ。
