...
|---|---|---|
| 2026-08-18 | Policy群の規範文を代用指標(バッククォート識別子・大文字定数・数値閾値の有無)で仕分け、**その数字を検算せずに計画の前提にした**。抽出し直して初めて崩れた | 自分(次工程で消費したため) |
| 2026-08-18 | session-context.shの出力全量を確認しただけでは不十分で、Claude Code側では先頭2,000文字しか文脈へ渡っていなかった | 自分(実際のhook退避ファイルと消費側コードを確認) |
| 2026-08-19 | AC5を所有者yoshiで確認して成功扱いしたが、実際の消費者dev-investigateではWAL用ファイルを開けなかった | 自分(ssh quory-investigate経由で再検証) |
