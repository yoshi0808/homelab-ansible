...
<!-- EXEC-002 -->
**実効的な境界は、承認の規則ではなく能力の不在で作られている。** `pve1` / `pve2` / `authy` / `quory` / `sophos-fw` へ、ansyは書込のできる接続手段を持たない。届くのは read 専用の forced command dispatch だけで、そこに書込の語彙は1つも無い。**本書の承認区分は、届く相手についてしか意味を持たない。**
...
<!-- EXEC-005 -->
**この形を過渡期として受け入れている**(Yoshinobu、2026-08-18)。`id_ann` は到達できる相手(`monnie` / `sandbox` / `ansy`)と到達できない相手の両方の `group_vars` から指されており、**鍵を用途で分けていないため「鍵の不在」で境界を表現できない**。分ける判断は、`monnie` の監視スタックが安定し開発側から触らなくなった時点で行う。**到達経路の候補は `monnie` の VM を複製して開発用の監視環境を作ることである**(Yoshinobu、2026-08-18)。開発側が本番の `monnie` へ触る理由が消えれば、`id_ann` を到達できる相手だけの鍵にできる。**「安定を待つ」ではなく、複製で先に切れる連鎖である。**
...
| 到達手段が無い | `pve1` / `pve2` / `authy` / `quory` / `sophos-fw` | ansyが書込のできる接続手段を持たない(内訳と壊れ方は EXEC-003) |
