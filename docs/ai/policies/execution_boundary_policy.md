...
<!-- EXEC-002 -->
**実効的な境界は、承認の規則ではなく能力の不在で作られている。** `pve1` / `pve2` / `authy` / `quory` / `sophos-fw` へは、ansyが認証情報を1つも持たない。届くのは read 専用の forced command dispatch だけで、そこに書込の語彙は1つも無い。**本書の承認区分は、届く相手についてしか意味を持たない。**
...
| 到達手段が無い | `pve1` / `pve2` / `authy` / `quory` / `sophos-fw` | ansyが認証情報を持たない |
