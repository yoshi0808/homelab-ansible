...
環境事実と実装詳細は対応Contextを参照し、競合時は本Policyを優先する。**実際に強制している機構は、実行中のプラットフォームの権限設定そのものが正本であり、値を本書へ写さない。どのファイルがそれかは各入口(`CLAUDE.md` / `AGENTS.md`)が持つ。**
...
<!-- EXEC-012 -->
**この区分は、各入口が名指しする権限設定のすべてと対応させて維持する。** 片方だけ変えるとドリフトする。**プラットフォームが増えれば、対応させる先も増える。**
