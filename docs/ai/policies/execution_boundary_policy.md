...
環境事実と実装詳細は対応Contextを参照し、競合時は本Policyを優先する。**実際に強制している機構は、実行中のプラットフォームの権限設定そのものが正本であり、値を本書へ写さない。どのファイルがそれかは入口 `AGENTS.md` が持つ。**
...
| `monnie` | **同上**(`id_ann` の削除で閉じた、2026-08-19)。**読み取り専用の名前付きチェックは `monnie-investigate` で今も通る**。一覧はカタログ(`docs/ai/reviews/dev_prod_boundary/2026-08-03_008_phase3_check_catalog.md`。Loki横断ログ調査は§9) | 同上 |
...
**quory 側に登録された鍵を、ansy と同じ判断で消してはならない。** quory の Semaphore に登録された鍵(inventory の `ssh_key_id` が指すもの)は本番の実行能力そのものであり、ansy で鍵を消して無害だったのは ansy が本番ジョブを走らせないからにすぎない。
...
| それ以外 | `ansy` / `sandbox` | 家庭向けサービスを提供せず、内容はGitから再現可能か、失っても停止を招かない観測データである |

<!-- EXEC-011 -->
**`sandbox` は壊れてよいものとして用意されている**(Yoshinobu、2026-08-06)。監視対象ではなく、他に何もこのホストへ流さない。

<!-- EXEC-012 -->
**この区分は、`AGENTS.md` が製品ごとに名指しする権限設定のすべてと対応させて維持する。** 一部だけ変えるとドリフトする。**プラットフォームが増えれば、対応させる先も増える。**
...
| 区分 | 扱い |
|---|---|
| `git commit` / `git push` | **Yoshinobuの明示承認を会話で得てCoordinatorが実行する。既定はcommitとpushを1つの承認単位とし、続けて実行する**(Yoshinobu決定、2026-09-05、承認経路の明確化2026-09-08)。**commitで止めるのは、Yoshinobuが明示したときか、よほど大規模な変更のときだけである。** 実行前にstageした内容の分類とcommitメッセージ案を提示する。Yoshinobuがその提示後に依頼または了承したことを実行権限とし、CLIの追加承認UIを権限の根拠にしない。**subagentは承認の有無にかかわらず行わない。** |
...
| `soft_deny` / `hard_deny` に該当する操作 | Coordinatorの承認では通らない。harnessのブロックはYoshinobu本人のintentのみ解除可。発火したらYoshinobuへ上げる。`git commit` / `git push` は1行目の会話上の承認で扱い、本行には含めない |
...
<!-- EXEC-060 -->
**人間の判断を求める入口を増やさない。** ansyでYoshinobuへ確認する実行操作は `git` の確定だけとし、本番実行の承認はquory側で押す。判断を要しない確認が混ざるとゲートは薄まる。`git` の確定は会話上の明示承認、quory側の本番実行はSemaphoreの操作をそれぞれ根拠とし、同じ判断への追加UI確認を必須にしない。**方針・採否・安全境界の変更をYoshinobuへ確認することは、ここでいう実行操作の入口に当たらない。**
...
<!-- EXEC-083 -->
**この機構を変更したときの確認手順も `docs/ai/core.md`「安全機構がブロックしたとき」を正本とする。**

## 8. 変更履歴

| 版 | 日付 | 内容 |
|---|---|---|
| v1.9 | 2026-09-23 | 冒頭で強制機構の所在を持つ文書として挙げていた「各入口(`CLAUDE.md` / `AGENTS.md`)」を、入口 `AGENTS.md` の1本へ改め、EXEC-012の「各入口が名指しする権限設定」も「`AGENTS.md` が製品ごとに名指しする権限設定」へ揃えた(Yoshinobu決定)。Claude Codeが `AGENTS.md` を直接読むようになったため `CLAUDE.md` を廃止し、製品ごとの強制機構の所在は `AGENTS.md` が製品別に並べる形にした。v1.7の記述は当時の事実として残す。EXEC番号の新設・退番はない。 |
| v1.8 | 2026-09-08 | EXEC-030の`git commit` / `git push`について、**stage内容とcommitメッセージ案の提示後に得た会話上の明示承認を実行権限とし、CLIの追加承認UIを権限の根拠にしない**形へ明確化した(Yoshinobu決定)。agmsg monitorと共有するCodex threadではheadless bridgeがapp-serverの承認要求を先に拒否し、スマホへUIが届かないことをCodex CLI 0.153.2 / 0.153.4の両方で確認したためである。commitとpushを1承認単位とすること、事前提示、subagent禁止は変更しない。EXEC-060も同じ判断への追加UI確認を要求しない表現へ揃えた。EXEC番号の新設・退番はない。 |
| v1.7 | 2026-09-06 | 本書が名指ししていた `.claude/settings.json` を、EXEC-001前文・EXEC-012・EXEC-083の3箇所から外し、**実行中のプラットフォームの権限設定**を指す形へ改めた。本リポジトリは特定ベンダーのAIを前提にしないため、強制機構を1製品の1ファイルへ固定していると、Coordinatorが別のCLIで動いた瞬間に本書の記述が事実と食い違う。どのファイルが強制機構かは各入口(`CLAUDE.md` / `AGENTS.md`)が持ち、本書はそこを指すだけとした。EXEC番号の新設・退番はない。 |
