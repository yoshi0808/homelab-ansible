...
**改訂6(2026-08-08、診断そのものの欠陥による差し戻し)**: 改訂5適用後もquory実機で`accept-request`が失敗し(`error: unexpected internal failure (StoreError, root=FileExistsError, errno=17)`)、状態が`submitted`のまま残った。Operatorが取得した`getfacl`は`mask::rw-`・`#effective`切り詰め無しで、改訂4/5のACL/作成モード是正自体は効いていることを示していた。**この差し戻しの論点は`accept-request`失敗そのものの修正ではなく、診断出力自体の欠陥** -- `lifecycle._root_cause()`は`__cause__`/`__context__`連鎖の最深部(innermost)1つだけを返す設計だったため、`store.append_event()`の`except FileExistsError:`(正常な通過点 -- 「他identityが既に作った続きに追記する」という制御フローであって失敗ではない)の中で別の例外が起きると、その最深部は正常な通過点である`FileExistsError`自身になり、**その内側で実際に起きた失敗が握りつぶされて報告されない**という構造的欠陥があった。是正は`_root_cause()`を削除し、`_exception_chain()`(連鎖の**全リンク**を外側から内側まで順に返す、循環耐性・長さ上限あり)へ置き換え、`_describe_failure()`が全リンクのクラス名(+`OSError`系は`errno`)を並べて出力するようにした。**この差し戻しでは`accept-request`失敗そのものの原因究明・修正は行っていない**(依頼文の明示的な指示どおり)。該当節(§3・§6・§8)を更新した。`docs/ai/roles/operator.md`・`docs/ai/context/operations/operator-request-channel.md`・`2026-08-08_002_plan.md`・`docs/ai/status.md`・`.claude/settings.json`はこの改訂でも一切触れていない。
...
**問題**: 上記バグの原因特定には、Coordinatorがquory上のOperatorへ`getfacl`の実行を依頼する必要があった。`run_entrypoint()`が出力していたのは外側の例外(`StoreError`)のクラス名だけで、その下の原因(`PermissionError`)は握りつぶされていた。`store.append_event()`自身の`StoreError`メッセージは実際には安全な形で原因のクラス名を含んでいた(`"cannot open event log: {}".format(type(exc).__name__)`)が、`run_entrypoint()`は`str(exc)`を一切出力しない設計だったため、この情報も一緒に失われていた。

**現在の実装**: `oprc/lifecycle.py`の`_exception_chain(exc)`が`__cause__`/`__context__`を外側から内側まで辿り、連鎖の**全リンク**をその順序のまま返す(循環耐性あり、`_MAX_CHAIN_LENGTH=8`件で打ち切り)。`_describe_failure(exc)`はこの全リンクそれぞれのクラス名(`OSError`系のリンクは`errno`(整数のみ)も付す)を`<-`で連結して1つの説明文字列を組み立て(`_MAX_DESCRIPTION_LENGTH=500`文字を超えると切り詰め)、`str(exc)`・`.filename`/`.filename2`はどのリンクについても一切読まない。`run_entrypoint()`はこの説明を`error: unexpected internal failure (<説明>)`という1行にしてstderrへ出力する。`_root_cause()`という関数はもう存在しない。

**最初の実装が不十分だった経緯(改訂6)**: 当初は全リンク方式ではなく、`_root_cause(exc)`という連鎖の**最深部1つだけ**を返す関数を実装していた。この設計は、`store.append_event()`の`except FileExistsError:`(「他identityが既に作った続きに追記する」という正常な制御フローであって失敗ではない)の中で別の例外が実際に起きると破綻する -- 最深部を辿ると、その正常な通過点である`FileExistsError`自身に行き着き、**その内側で実際に起きた失敗が握りつぶされて報告されない**。quory実機で`error: unexpected internal failure (StoreError, root=FileExistsError, errno=17)`という、まさにこの構造で握りつぶされた診断が実際に発生し(`accept-request`が`StoreError`で失敗、状態は`submitted`のまま変化せず)、この改訂の契機になった。是正として`_root_cause()`を削除し、`_exception_chain()`/`_describe_failure()`(全リンク方式)へ置き換えた。
...
**再確認条件の充足状況**: 全リンクのクラス名と(`OSError`系は)`errno`を`_describe_failure()`が出力に含める。例外メッセージ本文・path・payload・検出値はどのリンクについても一切読まない(`.filename`/`.filename2`を明示的に避けている)ことをコードとdocstringの両方に明記し、疑似secretをメッセージ本文へ埋め込む既存テスト(review差し戻しで追加)が是正後も全件成功することを確認した(§6参照)。循環耐性・チェーン長の上限(8件)・説明文字列の長さ上限(500文字)・`__cause__`優先・`OSError`以外にerrnoが付かないことは、`test_lifecycle.py`の`DescribeFailureExceptionChainTests`(7件)で個別に検証されている。
...
- **例外連鎖の可視化(全リンクのクラス名・errno)の確認**: `_exception_chain()`/`_describe_failure()`(全リンク方式、改訂6で`_root_cause()`から置き換え)を、ホワイトボックス(`test_lifecycle.py`の`DescribeFailureExceptionChainTests`、7件 -- 単一リンク・`except FileExistsError:`内で別例外が起きる形の2リンク再現・`__cause__`優先・循環耐性・チェーン長上限・説明文字列長上限・`OSError`以外にerrnoが付かないこと)と3エントリポイントそれぞれの2件の両方で検証した。3エントリポイント側は、改名後の`test_chain_reports_both_outer_and_wrapped_exception_class_name_and_errno`が`store.append_event()`が実際に使う形(`except OSError as exc: raise StoreError(...)`、Pythonの暗黙連鎖`__context__`を利用)で`PermissionError(13, "Permission denied")`を`StoreError`越しに送出し、stderrに外側クラス名(`StoreError`)・内側クラス名(`PermissionError`)・`errno=13`のすべてが現れることを確認し、`test_intermediate_exception_inside_a_control_flow_except_handler_is_not_hidden`が`store.append_event()`の`except FileExistsError:`内で別例外が起きる実際の形を再現して、正常な通過点である`FileExistsError`(errno=17)だけでなくその内側の実際の失敗(`PermissionError`、errno=13)も両方stderrに現れることを確認した。いずれの経路でも例外メッセージ本文(`"Permission denied"`・`"cannot open event log"`・`"File exists"`)はstdout/stderrに一切現れない。既存の疑似secret混入テスト(差し戻し2回目で追加)も是正後の出力形式で引き続き全件成功することを確認済み(§3参照)。
...
13. **`lifecycle.run_entrypoint()`が報告する診断が、正常な制御フロー分岐(`store.append_event()`の`except FileExistsError:`)の内側で起きた実際の失敗を握りつぶす構造的欠陥は是正済み(2026-08-08、本改訂=改訂6)。** 改訂5適用後もquory実機で`accept-request`が`error: unexpected internal failure (StoreError, root=FileExistsError, errno=17)`で失敗し続けた(Operatorが取得した`getfacl`は`mask::rw-`・`#effective`切り詰め無しで、項目11・12のACL/作成モード是正自体は効いていることを示していた)。原因は項目11で追加した`_root_cause()`が`__cause__`/`__context__`連鎖の**最深部1つだけ**を返す設計だったこと -- `except FileExistsError:`は「他identityが既に作った続きに追記する」という正常な制御フローであって失敗ではないが、その内側で別の例外が実際に起きると、最深部を辿った結果はその正常な通過点である`FileExistsError`自身になり、**実際に起きた失敗が報告されない**。`_root_cause()`を削除し、連鎖の全リンクを外側から内側まで順に返す`_exception_chain()`/`_describe_failure()`(循環耐性・チェーン長上限8・説明文字列長上限500文字)へ置き換えた。`test_lifecycle.py`に`DescribeFailureExceptionChainTests`(7件)を、3エントリポイントそれぞれに`test_intermediate_exception_inside_a_control_flow_except_handler_is_not_hidden`を追加し、`store.append_event()`が実際に踏んだ形(`except FileExistsError:`内で別例外が起きる)を再現して、`FileExistsError`だけでなくその内側の実際の失敗も両方stderrに現れることを確認した。既存の`test_root_cause_class_name_and_errno_survive_a_wrapped_exception`は、検証している性質(単一の根本原因ではなく全リンク方式の出力であること)に合わせて`test_chain_reports_both_outer_and_wrapped_exception_class_name_and_errno`へ改名した(検証内容そのものは変更していない)。**`accept-request`失敗そのものの原因究明・修正はこの改訂でも対象外のまま(未解決)。**
