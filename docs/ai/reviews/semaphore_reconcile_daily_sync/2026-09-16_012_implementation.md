...
改訂: 2026-09-16(差分レビュー `_013_diff_review.md` Critical 1 / Major 1 / Suggestion 2 / dead code 1、受入検証 `_015_test_result.md` Critical 1 / Minor 2、本番probe incidentを反映)
...
- 本番probeでP0-13が`semaphore_templates_report_dir`未定義となったため、collection playのprobeより前に`semaphore_templates` roleのno-op entry pointを`public: true`で読み込み、そのrole defaultsをplay 1へ公開する。パス値は同role defaultsを唯一の正本として引き続き使う。
...
| AC16 | `test_phase2_helpers.py`の20h40 fresh、24h/44h40 stale、欠落/非通常ファイル/不正JSON/未来時刻。task-flow scenario Jは実際の`playbooks/deployment_drift_check.yml`を`p0_13_scope_contract` tagでisolated localhost実行し、play 1のrole defaults公開と`semaphore_templates_report_dir`参照をassert | helper fixtureとplaybookのplay 1スコープassertを確認。隔離runではcollection/probe本体はtagで実行せず、実ホスト/APIへ接続しない。stat/slurp実タスクの権限・読取失敗fixtureなし（Testerへ回す） |
...
- `python3 roles/semaphore_templates/tests/task_flow/run_task_flow_tests.py`: 10 scenarios passed。scenario Iは最終diff上限を、scenario Jは実際の`deployment_drift_check.yml`を隔離localhostで実行してplay 1の変数スコープ契約を確認。
...
- `ansible-playbook playbooks/deployment_drift_check.yml -i localhost, --limit localhost --tags p0_13_scope_contract -e deployment_drift_check_hosts=localhost -e reports_base_dir=/tmp/deployment-drift-scope-test`: actual playbookのscope test passed。対象tagはno-op role defaultsの読み込みとmarker path assertだけで、collect/probe/report taskは実行していない。
- P0-13変更対象を含む`ansible-lint --profile min playbooks/deployment_drift_check.yml roles/deployment_drift_check/tasks/semaphore_probe.yml roles/semaphore_templates/tasks/noop.yml`は、既存の`roles/deployment_drift_check/roles/common_slack/tasks/notify.yml`欠落によるunskippable `load-failure`で停止。`ansible-playbook --syntax-check playbooks/deployment_drift_check.yml`と実playbookの隔離scope runは通過した。
...
P0-13関連タスクの`semaphore_templates_report_dir`参照を横断検索し、play 1の鮮度チェック以外に、同様に別role defaultsへ依存している参照は見つからなかった。実playbook検査は実ホスト/API禁止を守るため、localhost inventoryと専用tagでplay 1の変数スコープ契約に限定した。
