...
| **`journal-ssh` が配備され、各ホストで実際に行を返すか** | `playbooks/recovery_exec_setup.yml`(class G/P)と `playbooks/dev_investigate_setup.yml`(class Q)が次に quory の Semaphore から流れたとき。**quory 側の pull は `worktree-sync.timer` が1分間隔で自動追随するので待つだけでよい** — 要るのは Semaphore から該当2 playbook のテンプレートを起動することで、**テンプレートの起動は repo 外の操作であり Yoshinobu しかできない**(ansy から quory / pve へは届かない) | `ssh <host>-investigate "journal-ssh 24h"` が `denied:` を返さず rc=0 で**行を返す**こと(未配備の現在は `denied: unknown command` が返る、2026-08-03実測)。**空で返ったら正常とみなさない** — quory / authy / monnie は socket activation のため `sshd@*` を含めており、それが効いていなければ空になる。pve は sudoers の追加行が要るので、そちらが漏れると `sudo` の拒否として出る | `docs/ai/reviews/dev_prod_boundary/2026-08-03_008_phase3_check_catalog.md` §7 | 2026-08-03 |
...
| `skills/requirements-analysis/` に索引更新を成果物へ含める | playbookを増やす案件で `playbooks/README.md` の更新を成果物に含めれば、**Auditorの職掌を広げずに**「受入条件の充足」の検査で拾える。**当初はrole索引も対象だったが、`role-map.md` / `playbook-map.md` は2026-07-29に廃止済みで対象は1つに縮小した** | `docs/ai/reviews/incident_auto_capture_step2/progress.md` A-11、`docs/ai/context/ansible/repository-overview.md` L7 |
