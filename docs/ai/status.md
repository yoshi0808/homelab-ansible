...
| # | 項目 | 現在地 | 次にやること |
|---|---|---|---|
| 1 | **一次調査の起動経路をcallbackからバンドル起点へ切り替える**(Incident対応、**暫定**) | **実装・独立レビュー・Tester検証まで完了し、作業ツリーに未commitで載っている**(2026-07-31)。scratchでの受入条件はすべてPASS。案件記録は `docs/ai/reviews/incident_investigate_trigger/`(001〜005)、Auditorは条件付き受入で指摘1件を反映済み | **Yoshinobuの `git commit` / `push` → quoryで `git pull --ff-only` → `playbooks/incident_investigate_setup.yml` の再実行。** その後の観測2件は下のWatchが持つ。正本は `docs/ai/memory/incidents/2026-07-31_incident-investigate-callback-did-not-enqueue.md` |

**配備が済むまで、Semaphoreジョブが失敗してもバンドルは作られるが調査は起動しない。** `semaphore-495` がその状態で残っており、配備後の最初の1件として拾われる想定である。
...
| **一次調査が新しい起動契機(バンドル走査)で実発火するか** | 配備(commit / push / quoryでの `git pull --ff-only` / `incident_investigate_setup.yml` 再実行)の**後** | ①`reports/incidents/_investigations/semaphore-495.json` が現れること(現在このバンドルは未調査のまま滞留しており、最初の対象になる想定)②その後 `investigated_at` が変化しないこと(再投入されない)。**①②の両方**を観測しないと機構が効いた証明にならない。③quoryのキューディレクトリに以後キューファイルが**新規に現れない**こと(callbackが読み込まれていないこと)。scratchでの受入条件はすべてPASS済みで、残るのは本番経路だけである | `docs/ai/memory/incidents/2026-07-31_incident-investigate-callback-did-not-enqueue.md`「確認方法」、`docs/ai/reviews/incident_investigate_trigger/2026-07-31_004_test_result.md` | 2026-07-31 |
