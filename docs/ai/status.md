...
| # | 項目 | 現在地 | 次にやること |
|---|---|---|---|
| 1 | **開発と運用の境界を、設定でなく能力の不在で作る**(`docs/ai/reviews/dev_prod_boundary/`) | **Phase 1・2・3 完了(`incident_sync` 退役を除く)。** チェック一覧は 2026-08-03 に D5 承認済み(`..._008_phase3_check_catalog.md` が正本)。quory の新受け口 `dev-investigate`(20チェック)と class G / P の追加チェックが実配備され、**AC8 / 9 / 10 / 19 / 20 と R13b がすべて PASS**(`..._013_` / `..._014_`)。**Phase 4 の関門 S2・S3 は both 達成済みで、着手条件は揃っている** | **Phase 4(`ann@homelab-ansible` の削除)へ進める状態。** ただし着手前に `incident_sync` の退役が要り、それには**月次Knowledge振り返りの incident metrics をどこから採るか**の決着が要る(下記 Watch)。Tier 2(template配備物の内容比較)は保留、R9(Loki)は Q-B で読めるため不要と判断して落とした |
...
| **`incident_sync` の退役に伴い、月次Knowledge振り返りの「滞留カウント2件」を dispatch 経由へ移す** | `incident_sync` の退役に着手するとき(Phase 4 の前提。R14b) | `roles/knowledge_review/tasks/incident_metrics.yml` が読む5指標のうち、**代替が要るのは `mirror_bundle_total` と `bundles_new_since_last_eval` / `oldest_unreviewed_bundle`(=IC-021 の滞留検知)だけ**である。残る `mirror_heartbeat_age_seconds` / `mirror_stale` と `incident_evaluation.yml` L131-132 の `STALE` / `NO_DATA` 判定は**ミラー自身の健全性を見ており、ミラーごと消える**(代替不要)。移設先は timer が動かす Ansible 側 — ansy の `yoshi` として動き shell を持つため `ssh quory-investigate bundle-list` を叩ける。**`dev-investigate` 鍵を使うので Phase 4 で切れない。** `claude -p` 側は結果ファイルを読むだけで Bash 禁止のままでよい | `roles/knowledge_review/defaults/main.yml` L44-45、`roles/knowledge_review/tasks/incident_evaluation.yml` L128-134 | 2026-08-03 |
...
- **月次評価のために、バンドル本文をその場で組み立てて `claude -p` へ渡すこと** — 2026-08-03、Yoshinobuが**やらない**と決定した。`incident_sync` 退役後、`reports/incidents/quory/` は凍結スナップショットになり、以降の新規バンドルは件数と最古の情報しか月次セッションから見えない。Ansibleが評価直前にdispatchで本文を取得して置く案(常設ミラーではなく実行中だけ存在する形)を推奨したが、**却下**。理由は①**`claude -p` に読ませるためだけにアドホックな仕組みを足す対象ではない** ②中身が消えるわけではなく quory にあるので、8/26までにYoshinobu自身が目視で確認する ③**prompt・Policy・Skill・Context・承認ルールを大きく入れ替えた直後であり、振り返りをしても前提状況が不明である**。③は「読み手が履歴の前提ベースラインを特定できるか」という既存の判断軸(規範文書に経緯を書かない根拠と同じもの)を、振り返りそのものへ適用したものである。prompt側は「このセッションからは読めない」ことを明記済みで、捏造しない指示が入っている。
