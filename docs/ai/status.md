...
| **`tester_mode` が「廃止済み」と「現役」の二重の意味を持っている** | `docs/ai/policies/ansible_test_safety_policy.md` TS-003 は「`tester_mode`変数…は廃止済み」と言い切るが、`roles/common_slack/tasks/notify.yml` は `tester_mode` を今も有効な通知抑止フラグとして受理する(28〜45行)。規範側は `docs/ai/roles/tester.md` から案内を削除して `skip_notifications` へ寄せたが、**Policy本文の改訂とコードからの削除は未着手**。Policy改訂はYoshinobu必須 | `docs/ai/reviews/agent_definition_dedup/2026-08-02_004_review_post_move.md` Suggestion 2(Coordinatorが両方の現物で確認) |
