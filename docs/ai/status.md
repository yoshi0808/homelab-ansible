...
| **`incident_sync` を退役させると月次Knowledge振り返りの incident metrics が入力を失う** | `incident_sync` の退役に着手するとき(Phase 4 の前提。R14b) | `roles/knowledge_review/tasks/incident_metrics.yml` が `knowledge_review_incident_mirror_dir`(= ansy の `reports/incidents/quory/`)を読んでいること、`mirror_bundle_total` / `bundles_new_since_last_eval` / `oldest_unreviewed_bundle` / `mirror_heartbeat_age_seconds` / `mirror_stale` の5指標がそこに依存していることを現物で確認できる。**無人実行の `claude -p` は Bash 禁止・SSH 不可なので、新設した dispatch を自分では叩けない** — 採取するなら Ansible 側(timerが動かす play)で行う必要がある。**壊れ方は「値が null になる」であり、無人実行では気づきにくい** | `roles/knowledge_review/defaults/main.yml` L44-45、`docs/ai/reviews/dev_prod_boundary/2026-08-02_001_requirement.md` R14b | 2026-08-03 |
