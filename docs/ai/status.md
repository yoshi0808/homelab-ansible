...
**開発機(ansy)のSemaphoreが本番Slackへ通知する経路が開いたままである**。2026-09-16、ansyのschedule 24本がactiveのまま発火し、本番と同じ名前のERRORが7本Slackへ並んだ(**24本は `active: false` へ落として停止済み**)。**そのうえで、scheduleを止めても塞がっていない** — Semaphoreの組み込みアラート(`slack_alert: true`)が、タスクが失敗するたび本番チャンネルへ出す。**2026-09-16 10:21 JSTに `slack_alert` を `false` にして停止した**(`/etc/semaphore/config.json`、backupあり)。`email_alert` は `true` のままだが、この環境ではメールは動かない。 当面の回避は「ansyのSemaphoreのタスクとして走らせず、`ansible-playbook` を直接起動する」こと。記録は`docs/ai/memory/incidents/2026-09-16_dev-semaphore-schedules-armed-by-a-phase1-test.md`。
...
**Semaphore reconcile Phase 2: クローズ済み(2026-09-17)。後続の観測が金曜に1点だけ残る**。**04:00のscheduleが2026-09-17に自分で起動した**(#1132、`user_id: None` / `params: None`、全件無変更、通知なし。観測は`_021`)。独立監査は差戻し(`_020`)→是正→**条件付き受入(`_023`)**で、**5指摘を完全是正、3指摘(AC9系 / AC10 / AC13)は未充足のケースを記録して判断**し、再監査が求めた4点の記録訂正も反映した。**残る観測: 2026-09-18(金)の実行で、AC10の「金曜・停止あり」「金曜・停止0件」を確認する**(曜日を実行時に固定する手段が無く、`-e` は reserved-name guard が拒否するため自然実行を待つ)。**受容した未充足3件**(AC9系=Testerがquoryへ到達できずfixture代替 / AC13のschedule途中失敗=preflightが堅牢で安全に再現できない / AC13のevidence生成失敗=skipであって失敗注入ではない)の判断と理由は`_019` §6。**AC9系を実地で確かめられるのは、土曜のProxmox full patchを止めた週が来たときである。** 案件正本は`docs/ai/reviews/semaphore_reconcile_daily_sync/`。


**Role配分の入れ替え: pane 0 / pane 1 / 計画Reviewerは実起動を確認済み・Auditorの起動が未観測**。Coordinator=Claude Code(pane 0)、Implementer=Codex(pane 1常駐)、計画Reviewer=Codex(案件ごとfresh)、差分Reviewer / Tester=Claude Code subagent、Auditor=Codex(クローズ時1回)。ADR-013の3案件実験は1件も観測しないまま終了した。`.claude/agents/` からImplementerとAuditorの定義を削除し、Claude Codeでこの2役を起こせない状態にした。2026-09-14の`./new-session.sh --reset`で、pane構成・Codex Implementerのspawn・Coordinator↔Implementerのagmsg疎通を観測した。**2026-09-16、Implementerのモデルを`gpt-5.6-terra`へ上げた(Yoshinobu決定)** — 起動スクリプトは`.gitignore`済みのため、`new-session.sh`の`--model`の更新はYoshinobu側の操作である。**2026-09-16、計画Reviewerを`spawn.sh codex reviewer --fresh --model gpt-5.6-sol`でfresh起動し、4往復の査読まで観測した**(Implementerとは別プロセス・別モデル)。**`despawn.sh` は placement record が無いとして畳めず、ペインは残る。** **未観測はAuditorのfresh起動だけである**。判断は `docs/ai/adr/015-claude-coordinator-codex-implementer-allocation.md`、起動要件は `docs/ai/context/operations/agent-messaging.md` §10。
...
| 項目 | 内容 | 根拠 |
|---|---|---|
| **pve2のNVMe error-log増加が実害かどうか** | 2026-09の月次点検が `[要確認] pve2: NVMe error-logの累積数が増加しました` を出し、当該deviceは `error-log +2`(累積241件)・media error +0・消耗率1%・予備領域100%・重大警告なしである。**レポートの表示は正しく、事象が問題かどうかは未判定。** 増加が何を指すか(訂正可能なエラーの記録か、交換を要する劣化か)を切り分ける。急がない | Notionの月次ページ「ストレージ月次点検 2026-09」(Semaphoreジョブ #1111 が生成)。表示側の案件は `docs/ai/reviews/proxmox_storage_report_readability/2026-09-17_009_closeout.md` §4 |
