...
| # | 項目 | 現在地 | 次にやること |
|---|---|---|---|
| 1 | **一次調査の結果をSlack通知 + quory→ansy同期の即時実行**(R14) | **実装・独立レビュー・Tester検証・Auditor(条件付き受入、指摘3件反映済み)まで完了し、作業ツリーに未commitで載っている**(2026-08-01)。配備前に確かめられるAC(AC1/AC2/AC3/AC7/AC10)はすべてPASS。案件記録は `docs/ai/reviews/incident_investigation_notify/`(001〜005) | **Yoshinobuの `git commit` / `push` → quory・ansy双方への配備**(`playbooks/incident_sync_trigger_setup.yml` は両側に配備物を持つ)。配備後の観測5件は下のWatchが持つ |
...
| **一次調査の通知と即時同期が本番で成立するか**(AC4/AC5/AC6/AC8/AC9) | 配備(commit / push / quory・ansy双方への配備)の**後**、次にSemaphoreジョブが失敗したとき | ①`#alerts` に通知が1本届く ②ansyで `ansible-incident-sync.service` が**定刻でない**タイミングで起動する(journal)③**非通過側3つ** — forced commandが引数を無視する / 専用ユーザーが他のunitを起動できない / `recovery-exec` が鍵を読めない ④月次実行中の起動が exit 0 で静かにskipし `failed` にならない。**③は通過側だけ見て良しとしない。** 手順は一次記録が持つ | `docs/ai/reviews/incident_investigation_notify/2026-08-01_004_test_result.md` | 2026-08-01 |
...
| **Lesson昇格 2件** | ①decoy定型「`127.0.0.1` 閉ポート」は `ansible.posix.synchronize` に効かない(action pluginが `C.LOCALHOST` を特別扱いし黙ってローカル実行に落ちる)②subagentがharnessのブロックを迂回せず報告した**成功例** — `permission-boundaries-must-be-designed-not-prompted.md` は逸脱例しか持っていない | 同 A-3 / A-8 |
...
| **`ubuntu_vm_full_upgrade` の汎用 `systemctl --failed` 検査が広すぎる** | `roles/ubuntu_vm_full_upgrade/tasks/healthcheck.yml` L49-65 は対象ホストの**失敗unitを1件残らず critical に変える**。`playbooks/ubuntu_vm_full_upgrade.yml` L109 の対象は `dev_nodes:control_nodes:...` で **`control_nodes` = quory** が含まれるため、**パッチと無関係なunitの一時的な失敗が月次パッチを止める**。あわせて一次調査との間にフィードバック経路を作る — 調査unitが `failed` → この検査が拾う → Semaphoreジョブ失敗 → バンドル生成 → 一次調査 → Codex呼び出し(壊れていればまた失敗)。**現状の露出は小さい**(調査unitは毎分動き次周期で成功に戻るため `failed` は概ね60秒で消える)が、失敗が連続すると窓が広がる。**安易に対象を絞ると本物の失敗も見えなくなる側へ倒れる**ため、単独で設計する。次の月次発火は**2026-08-02** | 上記2ファイルの現物(2026-08-01実測)。ループの指摘はYoshinobu |
