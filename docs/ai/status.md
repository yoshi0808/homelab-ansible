...
| # | 項目 | 現在地 | 次にやること |
|---|---|---|---|
| 1 | **開発と運用の境界を、設定でなく能力の不在で作る**(`docs/ai/reviews/dev_prod_boundary/`) | **Phase 1・2 完了。Phase 3 は quory 側が完了し、保護対象ホスト側が未配備。** チェック一覧は 2026-08-03 に D5 承認済み(`..._008_phase3_check_catalog.md` が正本)。quory へ新設した `dev-investigate` 受け口(20チェック)は ansy から実接続で動作し、AC8 / 9 / 19 / 20 と R13b が PASS(`..._013_`)。**Phase 4 の関門 S2・S3 は both 達成済み** | **① Yoshinobu が quory で `ansible-playbook playbooks/recovery_exec_setup.yml -l quory` を流し、保護対象4ホストへ class G / P を配備する** ② その後 Tester が class G / P の AC8 / 9 / 10 を判定 ③ **`incident_sync` 退役は月次評価の入力問題(下記 Watch)を解いてから**。Tier 2(template配備物の内容比較)は保留、R9(Loki)は Q-B で読めるため不要と判断して落とした |
...
|---|---|---|---|---|
| **日次ドリフト検査が保護対象4ホストで赤くなる**(`authorized_keys` のエントリ数不一致) | 毎晩 00:40。**Phase 3 の class G / P を配備するまで続く** | `reports/drift/latest.json` の finding が `authy` / `monnie`(期待3・実際2)と `pve1` / `pve2`(期待2・実際1)だけであること。**これは検出として正しい挙動である** — repo 側の期待値を先に更新したため、未配備が可視化されている。配備すれば消える。他の finding が混ざっていたら別件 | `docs/ai/reviews/dev_prod_boundary/2026-08-03_012_review_phase3.md` | 2026-08-03 |
