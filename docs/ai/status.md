...
**dry_run衝突解消（配備/readback/通知観測待ち）** — `ubuntu_vm_full_upgrade`/`prometheus_update_check`はnamespaced operation enumへ移行し、旧変数をalways guardで拒否。Semaphoreの5 templateと2 scheduleも整合済み。実Semaphore配備・readback・通常inspect通知は未観測。案件: `docs/ai/reviews/update_playbooks_dry_run_collision/`。
