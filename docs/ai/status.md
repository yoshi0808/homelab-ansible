...
| # | 項目 | 現在地 | 次にやること |
|---|---|---|---|
| 1 | **`--check` の意味の一本化 — Round 2(14本の分類変換)** | Round 1は完了(実装・独立レビュー・Tester検証・Policy改訂・lint追加)。棚卸しの結果、**`risk-accepted` 17本のうち条件2(TS-009)を満たすのは3本だけ**(`cloudkey_cert_deploy` / `proxmox_backup_restore_verify` / `unifi_backup_fetch`)。**残り14本は条件1だけで分類されており、Policy自身の基準では `check-mode-native` が正しい。** 棚卸しの正本は `docs/ai/reviews/check_mode_semantics/2026-07-31_004_classification_audit.md` | 14本を1本ずつ `check-mode-native` へ変換し、通った時点で停止assertを外す(TS-032)。あわせて①`check-tester-gate.sh` に「`risk-accepted` の理由が条件2に言及していること」の検査を追加(**Round 1で入れなかったのは、14本が未変換のうちに入れるとpre-commitがリポジトリ全体で落ちるため**)②`cert_renew` と `cloudkey_cert_deploy` の線引き③`systemd_timers` / `recovery_push_drill_setup` は変換後ほぼゲート不要になるがラベルとして妥当か |

**Round 1で判明した重い事実**: `recovery_exec_setup` の `authorized_keys` 配布taskは、2026-07-08にquoryで3日間の本番SSH障害を起こした当のtaskである。同playbookのヘッダが `risk-accepted` の根拠として掲げる「影響は軽微・自己完結」は、この実績と矛盾している。Round 2の変換順序を決めるときの材料にする。
