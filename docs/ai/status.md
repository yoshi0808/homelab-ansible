...
| **経緯・根拠の除去を、残りの規範文書へ広げるか** | `docs/ai/roles/` 5本(2026-08-01)と `docs/ai/role-routing-index.md`(本案件)は済んだ。**残るのは `memory-classification.md`・`context-classification.md`・Policy群・`skills/`。** 落とし方の型は `docs/ai/reviews/norm_docs_rationale_removal/` が持つ — **落とす前に受け皿の実在を1件ずつ確認し、受け皿が無いものはADRを新設してから落とす**(role-routing-indexではmodel/effortの根拠がどこにも無く、ADR-010を新設した)。広げるかどうか自体が未決 | Yoshinobu表明(2026-08-02)、`docs/ai/reviews/norm_docs_rationale_removal/2026-08-02_001_requirement.md` 非ゴール |
| **通知抑止の既定を反転させるか**(実送信を明示的なopt-inにする等) | `roles/common_slack/tasks/notify.yml` は `skip_notifications` / `ansible_check_mode` のいずれかが真のときだけ送信を抑止する(`tester_mode` は2026-08-02に撤廃)。**`--check` による抑止は既に無条件で効いている**(2026-08-02に現物で確認)。**残る欠陥は「既定が送信」であること** — `--check` を伴わない検証では、抑止手段が「実行者が変数を渡す」か「decoyを置く」しかなく、どちらも選択を誤ると本番Slackへ出る。**Testerの検証が本番Slackへ到達した事象は3回目**(2026-07-26 / 07-29 / 08-01)だが、**3回目は送信経路を実際に走らせるのが目的で `--check` なし実行だったため、`--check` 抑止では防げない**。したがって対処は「実送信をopt-inにする」方向でしか効かない。`roles/common_slack` の全利用箇所へ波及するため案件として扱う | `docs/ai/memory/incidents/2026-07-29_tester-slack-notify-misfire.md`、`.../2026-08-01_tester-slack-decoy-did-not-contain-request.md`、`roles/common_slack/tasks/notify.yml` の現物 |
