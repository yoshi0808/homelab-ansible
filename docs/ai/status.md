...
| 項目 | 内容 | 根拠 |
|---|---|---|
| **global pause に TTL も未解除通知も無い** | `homelab-monitoring-pause` は明示的な resume まで無期限に続き、pause flag(空ファイル)は理由も期限も保持しない。2026-07-29、**8日間解除されずに自律復旧が全停止していた**ことが別案件の副産物として偶然発覚した。同系統の `homelab-mute-*` は 1〜240分の TTL 必須で自動失効する。対策候補は①global pause にも TTL を持たせる②pause 中を定期通知する③両方。**設計判断を含むため Incident 対応として即断せず起票**。<br>**制約(2026-07-29 Yoshinobu)**: 「しばらく都合があってこのサーバーを止めたい」という**長期の意図的停止も成立させる必要がある**。したがって `homelab-mute-*` と同じ短時間 TTL(最大4h)をそのまま持ち込むと正当な用途を壊す。**求めるのは自動失効ではなく「解除忘れの検出」**であり、②(pause 中の定期通知)か、長期を許す TTL + 期限接近の通知が筋。修正は今後検討。<br>あわせて「8日間の空白期間に自律復旧の発火を要する事象があったか」の遡り確認も未実施 | `docs/ai/memory/incidents/2026-07-29_global-monitoring-pause-left-on-8-days.md`、`roles/recovery_exec/files/homelab-monitoring-pause` |
| **外部到達性チェックの通知が原因を断定し、失敗理由を残さない** | **閾値は入れない(2026-07-29 Yoshinobu 判断、Coordinator 同意)。** `external_reachable()` の使用箇所は①ISP down/up の通知②`flush_notify_queue` の保留 の2つだけで、**アクションを一切起こさない**(コード自身が `監視のみ、発火しない` とログする)。ラダー側が5回連続失敗を要求するのは VM 再起動・failover という高価な操作を守るためで、consequence が違う以上、感度が違うのは正しい。むしろ**5×60秒の閾値を入れると1〜4分の実際の短時間断が完全に見えなくなる** — 通知専用の信号では1サンプル感度が情報量最大であり、欠陥ではない。<br>残る実害は2点。①通知文が「ISP または FW 断」と**原因を断定する**が、確かめたのは「HEAD が1回失敗した」ことだけ。2026-07-29 06:07 JST はこの文面により、存在しない外部障害の調査が起きた(真因は `apt-daily-upgrade` 直後の systemd daemon-reexec による**サービス再起動**。ホスト再起動ではない)。②**失敗理由(例外種別)を出力しない**ため、DNS / TCP / TLS / timeout のどれかを切り分けられず、当該調査も「未特定」で終わった。次回も同じ地点で止まり、複数回の傾向も突き合わせられない。<br>対応は「通知文を観測事実の記述へ変える」「失敗理由を1行ログに残す」の2点に限る。**②はコストが実際に出ている箇所** | `roles/recovery_probe/files/recovery-probe.py:107-118, 139-156, 446-460`、`docs/ai/reviews/slack_loki_investigation/2026-07-29_001_measurement.md` AC2 |
