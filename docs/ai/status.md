...
**着手候補: Semaphore を 2.19.12 へ上げる(2026-09-05 に検知が陽性を返した)** — `SAFE: Semaphore update check monthly`(毎月10日 20:00)を Yoshinobu が前倒しで手動実行し(**ジョブ #974**)、**ansy / quory の両方が `current: 2.19.8` / `latest: 2.19.12` / `status: update_available` を返して Slack へ送信された。** **これが新版検知の初めての陽性であり、機構が鳴くところまで実測できた**(異常ではないので play は失敗しない — 該当 task は skip)。**quory も 2.19.8 であることがここで確定した**(それまで未確認だった)。**上げるまでは 9/10 の定期実行も同じく鳴る。異常と読まないこと。**apt リポジトリが無く GitHub Releases からしか取れないため、この経路が唯一の検知手段である。**適用は手動**で、手順は `docs/ai/reviews/semaphore_upgrade/2026-08-18_002_manual_procedure.md`。**上げるときの持ち越し3点**: ①**community 版へ寄せる**(手順書§3。前提の `executor_image` 空 / `runner` 0件を直前に再測する) ②**AC5(読み取り経路)の検証は所有者 `yoshi` ではなく消費側の識別子で通す** — 前回の誤判定はここだった ③手順書のタイトルと版(`2.18.4 → 2.19.8`)を実際の版へ直す。案件記録は `docs/ai/reviews/semaphore_update_check/`。**同日、schedule の有効化ゲートを撤去した**(`docs/ai/reviews/semaphore_activation_gate_removal/`) — カタログが `active: true` と書けば1回の適用で有効になる
