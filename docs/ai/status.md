...
|---|---|---|---|---|
| **日次ドリフト検査の初回実行が、Phase 3 で増えた検査対象を通るか** | 次の 00:40(初回は 2026-08-04) | `reports/drift/latest.json` が `drift: 0` で終わること。Phase 3 で検査対象が増えている(quory の dispatch / helper 3本 + 既存の漏れ3本、`dev-investigate` の `authorized_keys`、保護対象4ホストの期待エントリ数 3/3/2/2)。**エントリ数が期待どおりであることは 2026-08-03 に実測済み**(全行 `command=` 始まり)だが、**日次ジョブが quory 上で通しで回った実績はまだ無い**(登録は 2026-08-03、初回発火は翌日) | `docs/ai/reviews/dev_prod_boundary/2026-08-03_012_review_phase3.md` | 2026-08-03 |
| **`sandbox` の HA failover が `--check` で通るか**(AC17) | pve1 が稼働している週末 | quory の Semaphore から `SANDBOX: Recovery ha failover (check)` を起動し、Phase 1(タグ・HA登録assert)と Phase 2(移行先選定)が**実データで通り**、`plan-only` として **rc=0** で終わること。`ha-manager crm-command relocate` が**発行されないこと**。**Phase 1 の完了条件から意図的に外した後追い項目**(pve1 の平日シャットダウン運用のため)。**Phase 4 で ansy から pve へ到達できなくなったため、起動は Semaphore からに限られる** | `docs/ai/reviews/dev_prod_boundary/2026-08-02_002_plan.md` §2、`2026-08-02_001_requirement.md` AC17 | 2026-08-03 |
...
| 項目 | 内容 | 根拠 |
|---|---|---|
| **調査面に `sshd` の journal を足すか** | 現在の dispatch カタログは、**境界そのものが乗っている transport(SSH)のログを見られない**。pve class の `journal-unit` enum は `pvescheduler` / `pvestatd` / `pve-cluster` / `corosync` / `pvedaemon` の5つで `sshd` を含まず、`journal-system` は `-p warning..err` で絞るため sshd の info レベルのメッセージ(MaxStartups throttling 等)が出ない。**Phase 4 で ansy から保護対象ホストへ届かなくなった以上、「なぜ SSH が切れたのか」を調べる手段は dispatch にしか無い。** 2026-08-03 に実際に `Connection reset by peer` の原因を特定できず、レート制限か経路の揺れかを判定できなかった。**チェックの追加には Yoshinobu の承認が要る(D5)** | `docs/ai/reviews/dev_prod_boundary/2026-08-03_023_test_result_phase4.md` Coordinator追記 |
| **Lesson昇格の候補 — 資格情報の列挙が漏れると境界が成立しない** | Phase 4 で `ann` の `authorized_keys` だけを数え、**`root` の側を見ていなかった**ため、鍵削除後も ansy から pve1 / pve2 へ root で入れる状態が残った(`..._024_finding_unenumerated_credential.md`)。持ち帰りは①数えるのは「自分が置いた鍵」でなく「**自分から届く鍵**」②**人が手で作った資格情報はどの inventory にも現れない**ため自動化の経路を追っても網羅にならない③漏れは「動いてしまう」形でしか現れず文書からは見つからない④**閉じる作業では検証手段を最後に手放す**。既存の `permission-boundaries-must-be-designed-not-prompted.md` と同じ面だが、あちらは「境界を prompt で作るな」、こちらは「**境界の前提である列挙が漏れる**」で別クラス | `docs/ai/reviews/dev_prod_boundary/2026-08-03_024_finding_unenumerated_credential.md` |
...
| **`deployed-hash` の対応表に `worktree-sync.sh` を足す** | 対応表は8件しか持たず、**同期スクリプト自身が古くなっても突合できない**。`git pull` は配備物を更新しないため(`docs/ai/context/operations/code-delivery-to-production.md` §2)、同期の仕組み自身がこの罠の対象外ではない。**単独では足さない** — 配備に commit → pull → Semaphore の一巡が要るため、次に dispatch を触る機会にまとめる(Phase 4 D6と同じ判断)。そのとき対応表の他の欠落も一緒に洗う。**チェック追加はD5でYoshinobuの承認事項** | 2026-08-03、Context執筆中にCoordinatorが気づいた。Yoshinobu合意済み(次の機会にまとめる) |
