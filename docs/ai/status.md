...
| # | 項目 | 現在地 | 次にやること |
|---|---|---|---|
| 1 | **Codexのexecpolicy allowlistが境界として機能していない**(Incident、状態: 原因判明・対応中) | 2026-07-31、実測で決着。**config.tomlに `[execpolicy]` というキーは存在しない**(実体はStarlarkの`.rules`)。しかも`.rules`でもallowlistは表現できない — catch-allが書けず、未一致コマンドはdecisionが付かずに通る。`codex exec`には承認系フラグが1つも無く承認ベースの既定denyも到達不能。**方針は「能力の不在へ一本化、execpolicyの復活は追わない」(Yoshinobu決定)。** repo側の是正は完了 — 死んだ`[execpolicy]`テーブルを`recovery_exec`/`incident_inspect`両方から削除、AR-069/071/073を改訂しAR-102を新設、ADR-009の該当2箇所を追記 | **残りは配備のみ。** commit/push → quoryで`git pull --ff-only` → `playbooks/recovery_exec_setup.yml` と `playbooks/incident_inspect_setup.yml`。**設定の削除は挙動を変えない**(元から読まれていない)ため急がない。**境界の再建は新規案件を立てず、下のNext「Codexの調査面を広げ、SSH鍵配布を縮小する」が本体。** 正本は `docs/ai/memory/incidents/2026-07-31_codex-execpolicy-allowlist-not-enforcing.md`(`codex execpolicy check`という決定論的な検証手段もそこに記載) |

**「障害の一次調査の自動化」は2026-07-31にクローズし、本番稼働に入った。** Semaphoreジョブが失敗すると、callbackが要求をキューへ入れ、quoryの毎分timerが証拠バンドルの到着を待って調査専用ユーザー `incident-inspect` のCodexを起動し、一次情報を `reports/incidents/_investigations/semaphore-<id>.{json,md}` へ書く。毎時 :07 の同期でansyへ渡る。**Codexの出力は一次情報であって原因の確定ではない**(真因の特定と修正の著述は開発側、正本は `docs/ai/core.md`「開発と運用の分離」)。規範は `docs/ai/policies/incident_capture_policy.md` **§3.5(IC-034〜IC-042)**、設計は `docs/ai/adr/009`(**Accepted**)、案件記録は `docs/ai/reviews/incident_auto_investigation/`(001〜008)。**実発火の観測2件はWatchが持つ。** この案件から派生したIncident 2件は未着手で、上のNowと下のNextが持つ。
...
| **`--check` の意味の多重定義を解く**(下の「通知抑止が破れる」と**同じ結び目**。片方だけ直しても解けない) | 実測(2026-07-31): `check-mode-native` 19本では `--check` は**シミュレート**を意味するが、`risk-accepted` 18本のうち**6本**(`recovery_exec_setup` / `recovery_io_setup` / `cert_renew` / `ca_trust_deploy` / `grafana_provisioning` / `incident_inspect_setup`)では `skip_notifications: "{{ ansible_check_mode }}"` として**「本適用はするが通知は抑止する」**を意味する。**同じフラグが逆の意味を持つため、「`risk-accepted` は `--check` で停止する」という単純な修正はこの6本を壊す。** 方向は「通知抑止を `ansible_check_mode` から切り離し、明示変数だけで決める」→ そのうえで `risk-accepted` は `--check` を渡されたら**明示的に停止**する(黙って適用しない)。**文書化では解けない** — 踏む側の注意力ではなくフラグの多重定義が原因であるため(2026-07-31 Yoshinobu指摘) | `docs/ai/memory/incidents/2026-07-31_subagent-unintended-deploy-risk-accepted-check.md`、`docs/ai/policies/ansible_test_safety_policy.md`、上記6本の `vars:` |
...
| **検証実行のSlack通知抑止が「変数の渡し忘れ」で破れる** | `roles/common_slack/tasks/notify.yml` は `tester_mode` / `skip_notifications` が真のときだけ抑止し、**渡し忘れた場合の既定が「本番へ送る」**。`--check` の有無は判定に入っていない。2026-07-29に再発(2026-07-26に前例)。既定を反転させるか `--check` を判定に加えるかは `common_slack` の全利用箇所へ波及するため案件として扱う。**上の「`--check` の意味の多重定義」と同じ結び目である** — `notify.yml` 自身は `ansible_check_mode` を見ないが、呼び出し側6本が `skip_notifications: "{{ ansible_check_mode }}"` で橋渡ししており、`--check` に2つ目の意味を与えているのはこの結線である | `docs/ai/memory/incidents/2026-07-29_tester-slack-notify-misfire.md` |
