...
| # | 項目 | 現在地 | 次にやること |
|---|---|---|---|
| 1 | **障害の一次調査の自動化**(Semaphoreジョブの失敗ごとにCodexが一次情報を残す) | 2026-07-31着手。requirement / ADR-009 / Step 0実測 / Policy改訂適用まで完了し、**Step 2(実装)に着手した**(U1=`incident_inspect` role、U2=`incident_investigate` role の2単位) | 実装 → 差分レビュー → 配備 → **`incident-inspect` として `codex login --device-auth`(ユーザー作成後。headlessでの成立は実証済み)** → Tester検証。**loginは実装の前提ではなく配備後の手順である**(順序を2026-07-31に訂正)。案件記録は `docs/ai/reviews/incident_auto_investigation/` |
