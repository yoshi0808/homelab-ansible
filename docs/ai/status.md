...
|---|---|---|---|---|
| **日次ドリフト検査が `drift: 0` へ戻るか** | 次の 00:40(2026-08-05) | `ssh quory-investigate "report-show drift latest.json"` の `drift_count` が 0、`findings` が空、`unverified_hosts` が空であること。**2026-08-04 の初回実行は `drift: 1` を返したが、これは真陽性**(`incident-investigate.py` が commit `7904ba2` 以前の版のまま配備されていた)で、同日 05:24 に `playbooks/incident_investigate_setup.yml` を再配備して解消済み — `deployed-hash incident-investigate` が repo の sha256 と一致することを実測。**この通知にエッジ抑止は無いため、解消できていなければ毎朝再通知される** | `docs/ai/reviews/dev_prod_boundary/2026-08-03_012_review_phase3.md` | 2026-08-04 |
| **ドリフト通知の「直し方」が、本番で実際に出るか** | 次にドリフトが検出されたとき(いつになるかは不明) | `ssh quory-investigate "report-show drift latest.json"` の finding に `playbook` / `run_host` が入っており、Slack本文に `直し方: ansible-playbook ... -l ...` の行が出ること。**decoyでは観測済みだが本番の通知では一度も出ていない。** 「直し方」行が出ない finding もありうる(流しても直らないものには意図的に書いていない)ので、**行が無いこと自体は異常ではない** — 異常なのは、`playbook:` を持つエントリの finding に行が出ないこと | `docs/ai/reviews/deploy_awareness/2026-08-04_003_review.md` | 2026-08-04 |
