...
| **Grafana alertの通知本文に実測値が入るか**(AC4) | 次にpacket error/dropが実発火したとき | Slack通知の本文に**機器名・port・発火時の実測値**が入っていること。**`{{ $values.A.Value }}` は公式ドキュメント上正しい式だが、実際に描画されるのは実発火時だけ** — 式が無効な場合Grafanaは本文へエラー文字列を描画する(静かに失敗せず、しかし人が読むまで気づかない)。健全なネットワークでは発火しないため人為的な検証ができず、意図的に案件クローズの必須条件から外した。**旧4ルール削除後なので同一事象で1通のみ**になることも併せて確認する | `docs/ai/reviews/grafana_provisioning/2026-07-30_001_requirement.md` §11 AC4、`roles/grafana_provisioning/files/alerting/unifi-switch-port-errors.yaml` | 2026-07-30 |
...
| **「Coordinatorは実ホストへのad-hocコマンド実行を行わない」を書き換える**(Yoshinobu提起 2026-07-30、`1+R`) | 現行の1行は**2つの別物を束ねている**。①安全(本番を勝手に触らない)は2026-07-30に機構へ移った — ホストで境界を引く決定(pve1/pve2/authy/sophos-fw/UniFiのみ確認必須)と`autoMode.soft_deny`で、**文章ではなくclassifierが止める**。②独立性(Testerが実ホスト検証を担う唯一のRole)は生きている。**①は規範として残っているが誰も守っておらず守る理由も無い状態**で、Grafana案件ではCoordinatorがmonnieへread-only調査を10数回実行した(事実の収集であり独立性は増えない)。一方②は同案件で実際に効き、CoordinatorがAC1/AC2を自分で実行したことを逸脱として記録できた。**線を「read-onlyか否か」ではなく「事実の収集か、受入の判定か」で引き直す**のがCoordinator案。**Yoshinobuは削除を提案したが、Coordinatorが書き換えを推奨し(b)後回しで合意した** | `docs/ai/roles/coordinator.md` L56、`skills/delegation-tier/SKILL.md` L135、`docs/ai/core-migration-map.md` C17-01、auto-memory `feedback_no_direct_ansible_adhoc_exec`(**4箇所が同時改訂対象。軸B該当で`1+R`必須**)。逸脱の一次記録は `docs/ai/reviews/grafana_provisioning/2026-07-30_008_test_result_step1.md`「工程上の逸脱」 |
