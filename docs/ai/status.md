...
| # | 項目 | 現在地 | 次にやること |
|---|---|---|---|
| 1 | **障害の一次調査の自動化**(Semaphoreジョブの失敗ごとにCodexが一次情報を残す) | 2026-07-31、**本番稼働開始**。実装・独立レビュー・配備・`codex login`・Tester実地検証(AC4/AC7/AC8/AC9 実測PASS)を経て、`homelab-incident-investigate.timer` を有効化(Yoshinobuが実行。**Coordinatorの実行はharnessにブロックされた**)。timer=enabled/active、service=正常終了を確認済み | **Auditorによるクローズのみ残る。** 実発火の観測2件はWatchが持つ(callbackが実Semaphore経路で動くか / AC6の実unit経路)。規範の正本は `docs/ai/policies/incident_capture_policy.md` §3.5、設計は `docs/ai/adr/009`、案件記録は `docs/ai/reviews/incident_auto_investigation/` |
| 2 | **Codexのexecpolicy allowlistが境界として機能していない**(Incident、状態: 調査中) | 2026-07-31にU0で発覚。`roles/recovery_exec` が配る `[execpolicy]` は `codex exec` でコマンド実行を阻止しない(allow-listに無い `id` が通った)。**本番経路そのものでは未確認**(同一内容の設定を複製した別CODEX_HOMEでの観測)。**文書と実態の乖離だけ同日に解消済み**(`AGENTS.md.j2` / `codex-config.toml.j2`)。境界は未修正 | **「順を追って対応」**(2026-07-31 Yoshinobu)。残りは⑴境界の作り直し(候補は`.rules`方式の実測 / 能力の不在で作る)⑵`autonomous_recovery_policy.md` **AR-069 / AR-071 / AR-073** が同じ前提を持つ(特にAR-073「sandboxとexecpolicyを別の防御層として扱い」は**現在1層**)。**Policy改訂はYoshinobu領域のため未着手。** 正本は `docs/ai/memory/incidents/2026-07-31_codex-execpolicy-allowlist-not-enforcing.md` |
