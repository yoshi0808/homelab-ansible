...
| # | 項目 | 現在地 | 次にやること |
|---|---|---|---|
| 1 | **開発と運用の境界を、設定でなく能力の不在で作る**(`docs/ai/reviews/dev_prod_boundary/`) | **Phase 1・2・3 完了。Phase 4 は Step 1〜4 完了。** チェック一覧は D5 承認済み(`..._008_phase3_check_catalog.md` が正本)。`incident_sync` は退役し実ホストの後始末まで済んだ。月次ジョブは `claude -p` を2本とも外して**きっかけの通知だけ**になり、`incident_capture_policy.md` は4段→3段へ改訂済み(IC-043 / IC-044 新設)。**この環境に無人の Claude Code セッションは1つも残っていない** | **残るは Step 5〜10。** ①**Yoshinobu が `ann@homelab-ansible` を pve1 / pve2 / authy → quory の順に削除**(pve1 が稼働している週末。最短 2026-08-08〜09)②ansy の `~/.ssh/config` 整理 ③`.claude/settings.json` の `autoMode` と `coordinator.md` を実到達範囲へ一致(**片方だけ変えない。Yoshinobu 承認**)④ansy Semaphore の縮小(20本削除・1本残す)⑤AC11 / 12 / 13 / 16 ⑥Auditor で案件クローズ |
