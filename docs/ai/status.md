...
**着手候補: quory の Semaphore を 2.19.12-community へ上げる(自動化は完成、ansy で検証済み)** — 2026-09-05 に検知が陽性を返し(ジョブ #974、`current: 2.19.8` / `latest: 2.19.12`)、**手動手順を playbook へ起こした**(案件 `docs/ai/reviews/semaphore_upgrade_automation/`)。`playbooks/semaphore_upgrade.yml` と `roles/semaphore_upgrade/`。Semaphore テンプレートに `UN-SAFE:` として登録済み、**schedule には載せていない**(2026-08-10 の「自動適用はしない」決定)。**ansy は既に 2.19.12-community で動いている。** **quory に残っているのは AC5(消費側の読み取り経路 — `homelab-semaphore-query` は quory にしか無い)と AC12(Semaphore ジョブ経由の起動 — ansy の Semaphore はジョブを実行できない)の2つだけで、どちらも ansy では原理的に踏めない。** 実行は `-e dry_run=false`(既定値は無く、省略は誤り)。**窓は Notion「バッチ処理工程管理表」で確認してから決める。** 版差そのものは無風である — `db/sql/migrations/` が 2.19.8 と 2.19.12 で同一(129ファイル)で**新規マイグレーション0本**、機能変更は全部 runner / executor 系で runner は0件。**マイグレーションが無い起動では `Migrations Finished` は出ない** — 8月の手動手順書 §5 の合否条件はこの版上げでは誤爆するため使わないこと(自動化側は `Server is running` と版の行で判定する)。**上げるまで毎月10日の検知は鳴り続ける。異常と読まないこと。**
