...
| **規範文書から経緯・根拠・lessons参照を落とす見直し**(Phase 2) | 規範文書には「今何を求めているか」だけを書き、経緯・日付つき履歴・`docs/ai/reviews/` や `docs/ai/memory/lessons/` への参照を置かない。2026-08-01に `docs/ai/roles/` 5本へ適用したが、**`docs/ai/role-routing-index.md` は対象から外れていた**(同ファイルにはモデル配分表の「根拠」列、medium確定の経緯、`claude-code-unattended-session-confinement.md` への参照が残る)。落とす前に、その経緯の受け皿(`git log` / `docs/ai/reviews/` / `docs/ai/adr/`)が実在するかを1件ずつ確認する。**対象を同ファイル単独に絞るか、`memory-classification.md`・`context-classification.md`・Policy群・`skills/` へ広げるかは入口で決める** | Yoshinobu表明(2026-08-02)。残存箇所の列挙は `docs/ai/reviews/agent_definition_dedup/2026-08-02_004_review_post_move.md`「What Looks Good」末尾 |
...
| **Lesson昇格 2件** | ①**decoyは「対象モジュールが実際にどう宛先を決めるか」を確かめない限り成立していると仮定してはならない。実例が2つ揃った** — `ansible.posix.synchronize` は `C.LOCALHOST` を特別扱いして黙ってローカル実行に落ちる(A-3)、`community.general.slack` は token のURL形状を無視し `domain` 未指定なら常に `hooks.slack.com` へ組み立てる(2026-08-01)。**2026-08-02に同型が再発した** — Testerが `tester_mode_full_removal` のAC2検証中、decoyのwebhook URLが無視され実HTTPが1件 `hooks.slack.com` へ到達した(garbage path、メッセージ配信なし)。**同じ原因分類が繰り返し検出されているので、Lessonを経由せず `skills/` へ直接落とす段階である**(`skills/incident-recording/SKILL.md` 運用ルール)。Tester側に前例を参照する経路が無いことが再発の条件になっている ②subagentがharnessのブロックを迂回せず報告した**成功例** — `permission-boundaries-must-be-designed-not-prompted.md` は逸脱例しか持っていない | 同 A-3 / A-8、`docs/ai/memory/incidents/2026-08-01_tester-slack-decoy-did-not-contain-request.md` |
...
| **29本の移行assertの撤去(R10)と、それに伴う TS-003 の削除** | `tester_mode` の識別子は撤廃済みだが、渡されると停止するassertが29本のplaybookに残っている。**外してよいのは、撤廃済みのコードが本番(quory)へ反映され、`tester_mode` を渡す経路が存在しないことを確認した後**である。順序を誤ると、かつてdrillがそうであったように、止まるはずだった経路が本番適用になる。撤去すれば識別子はリポジトリから完全に消え、`ansible_test_safety_policy.md` の TS-003 も削除できる | `docs/ai/reviews/tester_mode_full_removal/2026-08-02_001_requirement.md` R10 と「AC5の改訂」節 |
