...
| **`dry_run` の名前が Semaphore 自身の「Dry Run」と衝突している(残り2 playbook)** | Semaphore起動オプションの`dry_run`(=`--check`)と、playbook独自の同名変数が別の意味を持つ。**未着手で残るのは**`ubuntu_vm_full_upgrade`と`prometheus_update_check`、および独自`dry_run`を持つ稼働中schedule 2件。これらはscheduleの`environment`転記を誤ると本番適用へ変わるため、修正・配備・観測まで独立案件で扱う。先行した`semaphore_upgrade`はjob #1005で配備され、実templateからのSurvey消失まで観測済み。初回のquory UI Dry Runでは実`homelab-semaphore-query`/ACLとSQLite WAL/SHM補助ファイルを観測する | 完了した先行案件は`docs/ai/reviews/semaphore_upgrade_native_check_mode/`。残り2本の衝突説明は`roles/semaphore_templates/defaults/main.yml`第2バッチのコメント |
