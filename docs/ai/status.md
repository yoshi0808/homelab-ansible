...
| # | 項目 | 現在地 | 次にやること |
|---|---|---|---|
| 1 | **障害の一次調査の自動化**(Semaphoreジョブの失敗ごとにCodexが一次情報を残す) | 2026-07-31着手。requirement / ADR-009 / Step 0実測に加え、**Policy改訂(IC-002/003/005/007差し替え + §3.5 IC-034〜IC-042)と `core.md`「開発と運用の分離」を適用済み**。**実装は未着手** | **残るYoshinobu待ちは1件** — 調査専用ユーザー `incident-inspect` の `codex login`(ブラウザ操作。headlessなquoryでの手順は未確認)。これが済めばStep 2(実装)へ。案件記録は `docs/ai/reviews/incident_auto_investigation/` |
