...
**観測待ち: 切り離しの境界の修正が、Semaphore のジョブとして実際に効くこと(案件 `docs/ai/reviews/semaphore_upgrade_detach_reporting/`)** — **quory / ansy とも Semaphore 2.19.12-community で稼働している**(quory は2026-09-06 08:19、ansy は2026-09-05)。版上げそのものは終わった。**残っているのは2026-09-06の本番実行で壊れた2点の修正で、実装とレビュー4本は完了、`--check` の届かない範囲を踏むテストも入れたが、本番では未検証である。**<br>**未検証なのは AC1 と AC4 の2つで、どちらも ansy では原理的に踏めない**(ansy の Semaphore はジョブを実行できない、EXEC-052)。AC1=起動元のジョブが `running` のまま滞留せず `success` で確定する。AC4=停止までの窓に入ってきたジョブを待って守る。**2026-09-06 に押した3回は検証になっていない** — quory は git から取ったコードで走り、この案件は未commitだった(`#990` の `commit_hash` は `864e5c78`)。**次に押すのは push 後、`last_task` の `commit_hash` が変わったことを確かめてから。**<br>**押し方**: ①`UN-SAFE: Semaphore rollback`(既定値のまま、Semaphore 自身の Dry Run は外す)→ **20〜25秒後**に `SAFE: Time sync check` ②`UN-SAFE: Semaphore upgrade (apply)` を `dry_run=false`。**ロールバック直後は `#986` のような古い `running` 行が復活して次の版上げを preflight で止めるので、UI で stopped にしてから②へ進む**(この説明は通知と `fail_msg` に入れた)。**通知の文面(AC5〜AC7)は dispatch から読めない** — Slack の実画面が唯一の証拠である<br>**現行の commit 済みコードで実測した2件**(案件記録 `..._007_test_result.md`): ロールバックは `semaphore.db` を書き戻すため**ジョブ台帳が退避時点へ巻き戻り、それ以降の記録が消える**(#987 / #988 が消滅し #986 が `running` へ復活した)。停止の窓に入ったジョブは**実際に殺される**(08:18:50 開始の #991 が 08:19:01 の停止で巻き添え)。**どちらも ansy では現れない**
...
| **`dry_run` の名前が Semaphore 自身の「Dry Run」と衝突している(案件B、未着手)** | Semaphore の起動オプションにも `dry_run`(= `--check`)があり、**同じ名前が別の意味で2つある。** 現に稼働中のスケジュール `SAFE:Prometheus update check` は `environment: {"dry_run":"true"}` と `params: {dry_run: false}` を**同時に持つ**。2026-09-06、`UN-SAFE: Semaphore upgrade` の実行で実際に取り違えが起き、Semaphore 側の Dry Run を入れたため role が丸ごと短絡して**何も検証しない緑のジョブ**になった(#984)。<br>**こちらの `dry_run` は dry run ではない** — `semaphore_upgrade` の `dry_run=true` は DB を退避し `.deb` を落とし `transaction.json` を書く。「何も変えない」を意味する語を借りて別の概念を指している。**直すのはこちら側。** 推奨は `apply`(既定なし、`apply=true` で実行)。<br>**単独では軽いが影響範囲が広い** — `semaphore_upgrade` / `ubuntu_vm_full_upgrade` / `prometheus_update_check` の3 playbook、カタログ、**稼働中のスケジュール3件**に及ぶ。カタログ自身が警告しているとおり、`environment` の転記を誤ると**月次の dry-run が月次の本番 full upgrade になる**。Reviewer を通すこと | Yoshinobu提起(2026-09-06)。衝突の記述は `roles/semaphore_templates/defaults/main.yml` 第2バッチのコメントが既に持っている |
...
| **証明書の更新が、週次・期限駆動で実際に回った(2026-09-06 クローズ)** | **答え合わせは済んだ。** 日曜06:00の週次実行(ジョブ **#982**、06:00:01〜06:00:57、success)で、4ホストとも `days_left=13, needs_renewal=True` と判定して**実際に更新した**(ansy `changed=9` / monnie・pve1・pve2 各 `changed=12`、`unreachable=0`)。閾値15日に対する残り13日での発火であり、**期限駆動が実データで回ることが確認できた。****この行は次に読んだ者が消してよい。****承知の上の残存リスクだけが残る** — `prepare_ca_apply`(CA鍵のtmpfs展開)は `cert_needs_renewal` ではなく `not ansible_check_mode` だけでgateされているため、更新不要な週も毎回展開される(年12回→52回)。抑えるなら `issue_check` を全ホストで先に回す形になるが、playの順序組み替えを伴うので別案件 | 規範は `docs/ai/policies/cert_renew_policy.md` CERT-024。数値は `roles/homelab_cert_renew/defaults/main.yml`(valid 45 / threshold 15) |
