...
| ~~**一次調査の callback が実Semaphore経路で発火するか**(OQ2の決着)~~ **2026-07-31に否定側で決着 → Incidentへ移行** | — | **発火しなかった。** タスク#495(`cert_renew`)がERRORで終了したが `queue/` は空のままで、一次調査は起動しなかった。捕捉側は正常に動きバンドルは生成されている。切り分け済みの事項と残る候補は `docs/ai/memory/incidents/2026-07-31_incident-investigate-callback-did-not-enqueue.md` が正本。**この行は次にWatchを読む人が「まだ観測待ち」と誤読しないために残してある。原因判明後に消すこと** | 同Incident | 2026-07-31 |
