...
| **`dry_run` の名前が Semaphore 自身の「Dry Run」と衝突している(残り2 playbook)** | Semaphore起動オプションの`dry_run`(=`--check`)と、playbook独自の同名変数が別の意味を持つ。**`semaphore_upgrade`は独立案件として修正・commit/push済み(`374f12e`)、未配備**: 独自`dry_run`を削除し、UI Dry Run / `--check`をread-only診断へ一本化、旧引数は値によらずfail-closed、apply/rollback templateのSurveyから同変数を除去した。2名のReviewerがApproveし、localhost/decoy 12回とansy自身のnative checkで変更ゼロを確認済み。<br>**配備・観測待ちの制約**: quoryのtemplate catalog reconcileとSurvey消失を観測するまで、apply/rollback templateを実運用へ使わない。reading path有り側はstubでのみ確認済みで、実`homelab-semaphore-query`/ACLに対する初回はquoryのDry Runになる。`sqlite3`のread-only SELECTがWAL/SHM補助ファイルを生成しうるかも未確認なので、その初回実行では前後状態を観測する。<br>**未着手で残るのは**`ubuntu_vm_full_upgrade`と`prometheus_update_check`、および独自`dry_run`を持つ稼働中schedule 2件。これらはscheduleの`environment`転記を誤ると本番適用へ変わるため、Semaphore upgradeと混ぜず別案件で扱う | `docs/ai/reviews/semaphore_upgrade_native_check_mode/`。Incident: `docs/ai/memory/incidents/2026-09-06_semaphore-upgrade-dry-run-does-not-gate-rollback.md`。残り2本の衝突説明は`roles/semaphore_templates/defaults/main.yml`第2バッチのコメント |
