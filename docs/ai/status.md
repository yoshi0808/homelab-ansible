...
**「一次調査の起動経路をcallbackからバンドル起点へ切り替える」は2026-08-01にクローズした(暫定対応として)。** 本番配備後の観測で、滞留していた `semaphore-495` を含む**10件すべて**が古い順に1分1件で拾われ、成果物が生成された(`status: new` / `llm_rc: 0`)。非通過側(調査済みが再投入されない)・72時間の閾値・1周期1件・選定順序も本番で同時に確認済み。Auditorは条件付き受入で指摘1件を反映済み。**「暫定」の意味は、恒久の設計判断としてADR-009を書き換えていない**ことであり、動作が仮という意味ではない。案件記録は `docs/ai/reviews/incident_investigate_trigger/`(001〜006)、Incidentは `docs/ai/memory/incidents/2026-07-31_incident-investigate-callback-did-not-enqueue.md`(**解決済み**)。**残る観測2件は下のWatchが持つ。**
...
| **一次調査が、配備後に「自然発生した」失敗で発火するか** | 次にSemaphoreジョブが失敗したとき | 2026-08-01に処理した10件は**すべて切り替え前から滞留していたバンドル**であり、「捕捉がバンドルを作った直後に走査が拾う」経路(生成から数分以内、`task-time` の確定との競合が起きうる範囲)はまだ通っていない。`reports/incidents/_investigations/semaphore-<新しいid>.json` が、対応するバンドル生成から**概ね数分以内**に現れること。現れない場合は `journalctl -u homelab-incident-investigate`(要yoshi権限)を見る | `docs/ai/reviews/incident_investigate_trigger/2026-08-01_006_post_deploy_observation.md` | 2026-08-01 |
| **一次調査の失敗が本番で可視化されるか**(IC-038の本番実測) | 一次調査が実際に失敗したとき(LLM呼び出し失敗・タイムアウト・`task-time` 未確定のまま諦め等) | `_investigations/` に `status: failed` の成果物が残り、かつ `systemctl is-failed homelab-incident-investigate.service` が `failed` を返すこと。**2026-08-01時点で本番の成果物は10件すべて `status: new` であり、失敗経路は配備前fixtureでしか通っていない。** `SuccessExitStatus=75` により、flockによる正常なskipは `failed` にならない | 同上、および `docs/ai/reviews/incident_auto_investigation/2026-07-31_007_test_result.md` | 2026-08-01 |
...
| **Semaphoreの定期ジョブが `risk-accepted` へ `--check` を渡していないか**(commit `3bf9894` の直接の帰結。**残るは `proxmox_backup_restore_verify` と `cloudkey_cert_deploy` の2本** — `unifi_backup_fetch` は2026-07-31夜の定期実行が緑で通り、`--check` を渡していないことが確定した) | 次に各定期ジョブが発火したとき | 渡していればそのジョブが **rc=2 で赤くなる**(TS-030の停止assert)。Semaphore UIの失敗、または障害捕捉パイプラインが `reports/incidents/` にバンドルを作ることで判明する。**Semaphoreの設定はrepo外でAIから確認できないため、事前確認ではなく発火で検出する設計を選んだ**(2026-07-31、requirement §7 OQ3)。赤が出た場合の正しい対処は、assertを外すことではなく**そのジョブから `--check` を外すこと** | `docs/ai/reviews/check_mode_semantics/2026-07-31_001_requirement.md` §7 OQ3、`docs/ai/policies/ansible_test_safety_policy.md` TS-030 | 2026-07-31 |
