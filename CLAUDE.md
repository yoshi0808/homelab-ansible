...
実行許可・禁止の正本は [`docs/ai/policies/execution_boundary_policy.md`](docs/ai/policies/execution_boundary_policy.md) であり、**対象作業に関わらず作業開始時に読む**(他のPolicyと扱いが違う)。実際に強制している機構は Claude Code の設定(`.claude/settings.json`)である。Ansible実行を伴う作業を行うときは、安全分類の正本として [`docs/ai/policies/ansible_test_safety_policy.md`](docs/ai/policies/ansible_test_safety_policy.md) を確認する(他のPolicyと同様、対象作業のときだけでよく、作業開始時の読み込みには含めない)。
